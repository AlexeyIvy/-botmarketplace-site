...
STAGE="/home/botmarket/.local/share/botmarket/b15p1-final-launch-v0.2.0-stage"
RUNTIME_ROOT="/home/botmarket/botmarketplace-site"
BACKUP_ROOT="/home/botmarket/.local/share/botmarket/b15p1-final-launch-v0.2.0-backup"
...
LOCK_FILE="/run/lock/botmarket-b15p1-final-launch-v0.2.0.lock"
...
FREEZE_REL="docs/research/sc001-b15-p1-nonprice-collector-implementation-freeze-v0.1.4.json"
CONTRACT_REL="docs/research/sc001-b15-p1-nonprice-collector-implementation-contract-v0.1.4.json"
...
CANDIDATE_REL="docs/research/sc001-b15-p1-collector-launch-authorization-candidate-v0.2.json"
COLLECTOR_RESULT_REL="docs/research/sc001-b15-p1-nonprice-collector-v0.1.4-offline-selftest-result-v0.1.json"
CAPABILITY_RESULT_REL="docs/research/sc001-b15-p1-nonprice-source-capability-v0.2.2-current-live-result-v0.2.json"
RECOVERY_HOST_RESULT_REL="docs/research/sc001-b15-p1-v011-source-invalid-recovery-v0.1.2-host-result-v0.1.json"
...
EXPECTED_FREEZE_SHA="80928b0a11dac333b7844aa15bcdf399fd4db8739a4845e6118ebb576153b38c"
EXPECTED_CONTRACT_SHA="2ae6b87f59e8e495f57aca0eae5ca34fb40e920f01dd3abc746d8091f1ca138e"
...
EXPECTED_CANDIDATE_SHA="894457159a89ea9d701b8962c52aecd87e7805fc23d95a86ad74d9291d4ef437"
EXPECTED_COLLECTOR_RESULT_SHA="7cb9aa17fc820fe5bd88507614d273b52e442435de1e191b559053422b8c0c2c"
EXPECTED_CAPABILITY_RESULT_SHA="e1c770dec6c19e24922127839445646872498cab084478a60d085f96bafaa497"
EXPECTED_RECOVERY_HOST_RESULT_SHA="47f465c5f1afc5bb07ef422eda3e7fa9321aed7a00ed21f03ca53c6aeca13e7d"
EXPECTED_RECOVERY_REPORT_SHA="553a89d82dba67ee80166ccff32841b51ecbc25a9d8b47f5d84f6f6dd769e89b"
EXPECTED_SNAPSHOT_SHA="01cfa63b6001ec972c4ed87daba5bbd9fba932233b9e8323a49a6a5355e89647"
EXPECTED_AUTH_SHA="7c6a0296dc47f91a47d4747a0d29174338599a805b797427f19aa2e0c9d3b01c"
...
fail() {
  ...
  rollback
  echo "B15P1_FINAL_COLLECTOR_DEPLOYMENT_LAUNCH_V020_REVIEW"
  echo "reason=$reason"
...
[[ -f "$SAFE_SUMMARY" ]] || fail "v022_safe_summary_missing"
[[ ! -L "$SAFE_SUMMARY" ]] || fail "v022_safe_summary_symlink_forbidden"
...
if pgrep -f 'sc001_b15p1_nonprice_transferability_collector_v0_1_4[.]py' >/dev/null 2>&1; then
  fail "collector_v014_process_already_running"
fi
...
SOURCE_SHAS=(
  ...
  "$EXPECTED_CAPABILITY_RESULT_SHA"
...
printf '%s\n' "$selftest_stdout" | grep -q '^B15P1_NONPRICE_COLLECTOR_V014_SELF_TEST_PASS$' || fail "staged_selftest_pass_token_missing"
...
collector_path = Path(os.environ["B15P1_REPO_ROOT"]) / "research/sc001/sc001_b15p1_nonprice_transferability_collector_v0_1_4.py"
...
# Validate collector v0.1.4, current capability v0.2.2 and recovery v0.1.2
# immediately before the mutation boundary.
python3 - \
  "$STAGE/$COLLECTOR_RESULT_REL" \
  "$STAGE/$CAPABILITY_RESULT_REL" \
...
(
    collector_result_arg,
    capability_result_arg,
    recovery_host_arg,
    recovery_report_arg,
    snapshot_arg,
    safe_summary_arg,
...
collector_result = load(collector_result_arg)
capability_result = load(capability_result_arg)
...
assert collector_result.get("status") == "B15P1_NONPRICE_COLLECTOR_V014_COMPILE_AND_SELFTEST_PASS"
assert collector_result.get("package_integrity_ok") is True
assert collector_result.get("exit_code") == 0
assert collector_result.get("mandatory_test_count") == 32
assert collector_result.get("all_mandatory_tests_passed") is True
assert collector_result.get("runner_sha256") == expected_runner_sha
assert collector_result.get("library_sha256") == expected_library_sha
assert collector_result.get("implementation_freeze_sha256") == expected_freeze_sha

assert capability_result.get("status") == "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_COMMAND_V022_PASS"
assert capability_result.get("snapshot_sha256") == expected_snapshot_sha
cap_collector = capability_result.get("collector") or {}
assert cap_collector.get("runner_sha256") == expected_runner_sha
assert cap_collector.get("library_sha256") == expected_library_sha
assert cap_collector.get("implementation_freeze_sha256") == expected_freeze_sha
assert cap_collector.get("service_sha256") == expected_service_sha
cap_cov = capability_result.get("coverage") or {}
assert cap_cov.get("qualified_bybit_count") == 192
assert cap_cov.get("qualified_okx_count") == 192
assert cap_cov.get("qualified_both_venues_count") == 192
cap_bybit = capability_result.get("bybit_adapter") or {}
assert cap_bybit.get("collector_parser_version") == "0.1.4"
assert cap_bybit.get("parser_compatible") is True
assert int(cap_bybit.get("raw_withdrawMax_minus_one_rows") or 0) == 1007
assert int(cap_bybit.get("normalized_unlimited_rows") or 0) == 1007

assert recovery_host.get("status") == "B15P1_V011_SOURCE_INVALID_RECOVERY_V012_PASS"
assert recovery_host.get("v022_snapshot_sha256") == expected_snapshot_sha
assert recovery_host.get("recovery_report_sha256") == expected_recovery_report_sha
assert recovery_host.get("archived_file_count") == 134
...
assert sha(recovery_report_arg) == expected_recovery_report_sha
assert recovery.get("status") == "B15P1_V011_SOURCE_INVALID_RECOVERY_V012_PASS"
assert recovery.get("v022_snapshot_sha256") == expected_snapshot_sha
assert recovery.get("archived_file_count") == 134
...
assert anchors.get("library_sha256") == expected_library_sha
...
assert int(bybit.get("raw_withdrawMax_minus_one_rows") or 0) == 1007
assert int(bybit.get("normalized_unlimited_rows") or 0) == 1007
...
assert safe.get("snapshot_sha256") == expected_snapshot_sha
...
print("final_launch_v020_prerequisites = PASS")
print("recovery_archived_files =", len(archived_files))
print("snapshot_sha256 =", expected_snapshot_sha)
PY
...
UNIT_TMP="$RUNTIME_UNIT.b15p1.v020.tmp.$$"
rm -f "$UNIT_TMP"
...
AUTH_TMP="$AUTH_FILE.b15p1.v020.tmp.$$"
rm -f "$AUTH_TMP"
...
collector = root / "research/sc001/sc001_b15p1_nonprice_transferability_collector_v0_1_4.py"
...
print("runtime_prestart_v020_freeze_capability_authorization = PASS")
PY
...
require(state.get("stage") == "SC001-B15P1-NONPRICE-TRANSFERABILITY-COLLECTOR-V0.1.4", "state_stage_mismatch")
require(state.get("version") == "0.1.4", "state_version_mismatch")
...
require(manifest.get("stage") == "SC001-B15P1-NONPRICE-TRANSFERABILITY-COLLECTOR-V0.1.4", "manifest_stage_mismatch")
require(manifest.get("version") == "0.1.4", "manifest_version_mismatch")
...
report = {
    "schema": "sc001.b15.p1_collector_final_launch_verification.v0.2.0",
    "status": "B15P1_FINAL_COLLECTOR_DEPLOYMENT_LAUNCH_V020_PASS",
    "verified_utc": datetime.now(timezone.utc).isoformat(),
    "technical_retry_of": "v0.1.1 source-invalid attempt after Bybit withdrawMax compatibility fix",
