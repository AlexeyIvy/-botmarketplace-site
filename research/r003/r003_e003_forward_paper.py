"""R003-E003 forward paper tracker v0.1."""
...
def get_json(url: str, params: dict):
    req = Request(
        ...
        headers={"User-Agent": "botmarketplace-r003-e003-forward/0.1"},
    )
...
def fetch_klines(url: str, limit: int, label: str):
    ...
    while True:
        obj = get_json(
            url,
            {"symbol": SYMBOL, "interval": "1h", "startTime": start, "limit": limit},
        )
        ...
        valid = [
            r for r in obj
            if isinstance(r, list) and len(r) >= 7 and int(r[6]) < now_ms
        ]
        rows.extend(valid)
...
def fetch_funding():
    ...
    if not rows:
        return pd.DataFrame(
            columns=["funding_time", "funding_rate", "published_mark_price", "rate_type"]
        )
...
def build_common(spot, fut, mark):
    ...
    common = (
        common[common["open_time"] >= INCEPTION]
        .sort_values("open_time")
        .drop_duplicates("open_time", keep="last")
        .reset_index(drop=True)
    )
    if common.empty:
        return common
    common["close_time"] = common[
        ["spot_close_time", "fut_close_time", "mark_close_time"]
    ].min(axis=1)
...
def align_funding(funding, common):
    if funding.empty or common.empty:
        return pd.DataFrame(
            columns=[
                "funding_time", "funding_rate", "published_mark_price", "rate_type",
                "open_time", "close_time", "mark_close", "funding_mark"
            ]
        )
    ...
    x = x.dropna(subset=["open_time", "funding_mark"]).reset_index(drop=True)
    return x
...
def audit_data(spot, fut, mark, common, funding, funding_aligned):
    ...
    if common.empty:
        ...
        expected = int(
            round(
                (common["open_time"].iloc[-1] - common["open_time"].iloc[0]).total_seconds()
                / 3600.0
            )
        ) + 1
    ...
    enough_for_gate = len(common) >= 24
    if not enough_for_gate:
        status = "EARLY_FORWARD_SAMPLE"
    else:
        source_ok = all(
            s["coverage"] >= MIN_COVERAGE
            and (s["max_gap_hours"] is None or s["max_gap_hours"] <= MAX_GAP_HOURS)
            for s in stats.values()
        )
        common_ok = (
            common_coverage >= MIN_COVERAGE
            and (common_gap is None or common_gap <= MAX_GAP_HOURS)
        )
    ...
    return {
        ...
        "common_expected_hours": int(common_expected),
        "common_coverage": float(common_coverage),
        "common_max_gap_hours": common_gap,
...
def simulate(common, funding_aligned, fee, treatment):
    ...
    fa = funding_aligned[
        (funding_aligned["funding_time"] > init_close)
        & (funding_aligned["close_time"] <= z["close_time"].iloc[-1])
    ].copy()

    funding_map = {}
    for row in fa.itertuples(index=False):
        funding_map.setdefault(row.open_time, []).append(
            (float(row.funding_rate), float(row.funding_mark), row.funding_time)
        )
    ...
    rows = [{
        ...
        "nav_going_concern": q * spot0 + futures_cash,
        "pair_pnl": 0.0,
        "funding_cash": 0.0,
        "execution_fee": init_fee,
        "close_collateral_ratio": min_close_ratio,
        "intrahour_collateral_ratio": min_close_ratio,
        "rebalanced": True,
    }]
    ...
    for i in range(1, n):
        ...
        funding_cash = 0.0
        for rate, fmark, _ftime in funding_map.get(z.loc[i, "open_time"], []):
    ...
    last = path.iloc[-1]
    exit_fee = fee * (
        abs(float(last["q_btc"])) * float(last["spot_close"])
        + abs(float(last["q_btc"])) * float(last["mark_close"])
    )
    ending_gc = float(last["nav_going_concern"])
    ending_liq = ending_gc - exit_fee

    elapsed_years = max(
        (path["close_time"].iloc[-1] - path["close_time"].iloc[0]).total_seconds()
        / (365.25 * 86400.0),
        1.0 / HOURS_PER_YEAR,
    )
    elapsed_days = elapsed_years * 365.25
    cagr = (
        float(ending_liq ** (1.0 / elapsed_years) - 1.0)
        if ending_liq > 0 and elapsed_days >= 30
        else np.nan
    )
    ...
    vol = (
        float(path["net_return_gc"].iloc[1:].std(ddof=0) * math.sqrt(HOURS_PER_YEAR))
        if len(path) >= 24 * 30
        else np.nan
    )
    ...
    return path, {
        ...
        "cumulative_return_liquidation_adjusted": strat_cum_liq,
        ...
        "realized_execution_cost": float(cum_fee_realized),
        "hypothetical_terminal_exit_cost": float(exit_fee),
        "total_cost_including_hypothetical_exit": float(cum_fee_realized + exit_fee),
        "spot_turnover_including_hypothetical_exit": float(
            spot_turn + abs(float(last["q_btc"])) * float(last["spot_close"])
        ),
        "perp_turnover_including_hypothetical_exit": float(
            perp_turn + abs(float(last["q_btc"])) * float(last["mark_close"])
        ),
        "min_close_collateral_ratio": float(min_close_ratio),
        "min_intrahour_collateral_ratio": float(min_intrahour_ratio),
        "low_headroom_count_proxy": int(low_headroom_hours),
        ...
        "treasury_hurdle_cumulative_same_elapsed_time": float(hurdle_cum),
        "compensation_hurdle_cumulative_same_elapsed_time": float(comp_cum),
        "excess_cumulative_vs_treasury": float(strat_cum_liq - hurdle_cum),
        "excess_cumulative_vs_compensation_floor": float(strat_cum_liq - comp_cum),
    }
...
def write_waiting(outdir, reason, audit=None):
    ...
    if audit is not None:
        state["source_audit"] = audit
    (outdir / "r003_e003_run_state.json").write_text(
        json.dumps(state, indent=2), encoding="utf-8"
    )
    (outdir / "r003_e003_summary.md").write_text(
        "# R003-E003 Forward Paper\n\n"
        f"**Status:** WAITING_FOR_FORWARD_DATA\n\nReason: {reason}\n",
        encoding="utf-8",
    )
...
def run(outdir):
    ...
    if now <= INCEPTION + pd.Timedelta(hours=1):
        write_waiting(
            outdir,
            "The first common 1h bar after the frozen inception boundary is not yet fully closed.",
        )
        return
    ...
    (outdir / "r003_e003_source_audit.json").write_text(
    ...
    margin.to_csv(outdir / "r003_e003_margin.csv", index=False)

    init_close = baseline_path["close_time"].iloc[0]
    fe = funding_aligned[
        (funding_aligned["funding_time"] > init_close)
        & (funding_aligned["close_time"] <= baseline_path["close_time"].iloc[-1])
    ].copy()
    fe.to_csv(outdir / "r003_e003_funding_events.csv", index=False)

    monthly = monthly_from_path(baseline_path)
    monthly.to_csv(outdir / "r003_e003_monthly.csv", index=False)
    ...
    def pct(v):
        return "n/a" if pd.isna(v) else f"{float(v):.2%}"
    ...
    md += "No terminal strategy conclusion is allowed before the frozen evidence thresholds are met.\n"
...
def main():
    ...
    try:
        ...
        (args.outdir / "r003_e003_run_state.json").write_text(
            json.dumps(state, indent=2), encoding="utf-8"
        )
        raise
