...
STAGE = "SC001-B15P1-NONPRICE-SOURCE-CAPABILITY-REVALIDATION-V0.2.2"
PASS = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_PASS"
REVIEW = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_REVIEW"
SELFTEST_PASS = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_V022_SELF_TEST_PASS"
SELFTEST_REVIEW = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_V022_SELF_TEST_REVIEW"
...
SPEC = ROOT / "docs/research/sc001-b15-p1-nonprice-source-capability-revalidation-spec-v0.2.2.json"
FREEZE = ROOT / "docs/research/sc001-b15-p1-nonprice-source-capability-revalidation-freeze-v0.2.2.json"

RUNNER = ROOT / "research/sc001/sc001_b15p1_nonprice_transferability_collector_v0_1_4.py"
LIBRARY = ROOT / "research/sc001/sc001_b15p1_nonprice_transferability_lib_v0_1_4.py"
IMPLEMENTATION_FREEZE = ROOT / "docs/research/sc001-b15-p1-nonprice-collector-implementation-freeze-v0.1.4.json"
SERVICE = ROOT / "ops/systemd/sc001-b15p1-transferability-v0.1.4.service"
V014_BINDING_CORRECTION = ROOT / "docs/research/sc001-b15-p1-source-capability-v0.2.2-v014-binding-correction-v0.1.json"
V014_OFFLINE_RESULT = ROOT / "docs/research/sc001-b15-p1-nonprice-collector-v0.1.4-offline-selftest-result-v0.1.json"
SOURCE_INVALID_DIAGNOSTIC = ROOT / "docs/research/sc001-b15-p1-final-launch-v0.1.1-host-attempt-v0.1-diagnostic.json"
CREDENTIAL_SECURITY_CONTRACT = ROOT / "docs/research/sc001-b15-p1-read-only-credential-security-contract-v0.1.md"
...
def require_freeze() -> dict[str, Any]:
    ...
    if fr.get("status") != "FROZEN_BEFORE_B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_V022":
    ...
    expected = {
        ...
        "v014_binding_correction_sha256": sha256_file(V014_BINDING_CORRECTION),
        "collector_v014_offline_result_sha256": sha256_file(V014_OFFLINE_RESULT),
        "source_invalid_diagnostic_sha256": sha256_file(SOURCE_INVALID_DIAGNOSTIC),
        "credential_security_contract_sha256": sha256_file(CREDENTIAL_SECURITY_CONTRACT),
    ...
    if fr.get("old_v021_snapshot_reuse_allowed") is not False:
        fail("old v0.2.1 snapshot reuse must remain forbidden")

    offline = require_object(V014_OFFLINE_RESULT)
    if offline.get("status") != "B15P1_NONPRICE_COLLECTOR_V014_COMPILE_AND_SELFTEST_PASS":
        fail("collector v0.1.4 offline prerequisite status mismatch")
    if offline.get("all_mandatory_tests_passed") is not True:
        fail("collector v0.1.4 mandatory tests prerequisite mismatch")
    if int(offline.get("mandatory_test_count") or 0) != 32:
        fail("collector v0.1.4 mandatory test count prerequisite mismatch")
    return fr
...
def load_collector_library_v014():
    spec = importlib.util.spec_from_file_location(
        "b15p1_collector_lib_v014",
        LIBRARY,
    )
    if spec is None or spec.loader is None:
        fail("collector v0.1.4 library import spec unavailable")
...
def validate_bybit_collector_v014_parser(obj: dict[str, Any]) -> dict[str, Any]:
    module = load_collector_library_v014()
    try:
        parsed = module.parse_bybit_coin_info(obj)
    except Exception as exc:
        fail(
            "collector v0.1.4 Bybit parser rejected live coin-info: "
            f"{type(exc).__name__}: {exc}"
        )
    if not isinstance(parsed, list) or not parsed:
        fail("collector v0.1.4 Bybit parser returned no rows")

    raw_minus_one = 0
    result = obj.get("result") or {}
    rows = result.get("rows") or []
    if not isinstance(rows, list):
        fail("collector v0.1.4 Bybit parser source rows not list")
    for coin in rows:
        ...
        if not isinstance(chains, list):
            continue
        for ch in chains:
            if isinstance(ch, dict) and str(ch.get("withdrawMax") or "") == "-1":
                raw_minus_one += 1

    unlimited_rows = sum(
        1 for row in parsed
        if isinstance(row, dict) and row.get("max_withdrawal") == "UNLIMITED"
    )
    if unlimited_rows != raw_minus_one:
        fail(
            "collector v0.1.4 Bybit withdrawMax sentinel mapping mismatch: "
            f"raw_minus_one={raw_minus_one} unlimited_rows={unlimited_rows}"
        )

    return {
        "parser_version": "collector_v0.1.4",
        "parsed_chain_rows": len(parsed),
        "raw_withdrawMax_minus_one_rows": raw_minus_one,
        "normalized_unlimited_rows": unlimited_rows,
...
def bybit_spot_no_pagination_static_guard() -> None:
    function_source = inspect.getsource(bybit_live_usdt_spot_pairs)
    forbidden_markers = (
        "nextPage" + "Cursor",
        "\"cur" + "sor\"",
        "'cur" + "sor'",
    ...
    hits = [marker for marker in forbidden_markers if marker in function_source]
    if hits:
        fail(
            "Bybit Spot pagination logic unexpectedly present: "
            + ",".join(hits)
        )
...
def build_snapshot_fixture(
    ...
    return {
        ...
        "bybit_adapter": {
            ...
            "parser_compatible": True,
            "withdrawMax_minus_one_semantics": "UNLIMITED",
            "raw_withdrawMax_minus_one_rows": 1,
            "normalized_unlimited_rows": 1,
        },
...
def validate_snapshot_contract(obj: dict[str, Any]) -> None:
    ...
    if bybit_adapter.get("collector_parser_version") != "0.1.4":
        fail("snapshot Bybit collector parser version mismatch")
    if bybit_adapter.get("parser_compatible") is not True:
        fail("snapshot Bybit collector parser compatibility mismatch")
    if bybit_adapter.get("withdrawMax_minus_one_semantics") != "UNLIMITED":
        fail("snapshot Bybit withdrawMax sentinel semantics mismatch")
...
def write_selftest_manifest(status: str, error: str | None = None) -> None:
    ...
    atomic_json(
        ...
        {
            "schema": "sc001.b15.p1_nonprice_source_capability_revalidation_self_test.v0.2.2",
            "status": status,
            "error": error,
...
def self_test() -> int:
    try:
        ...
        parser_fixture = validate_bybit_collector_v014_parser(by_parser_fixture)
        if parser_fixture.get("normalized_unlimited_rows") != 1:
            fail("collector v0.1.4 Bybit sentinel parser fixture")
        ...
        bybit_spot_no_pagination_static_guard()
...
def live_run() -> int:
    try:
        ...
        by_parser = validate_bybit_collector_v014_parser(by_coin_obj)
        ...
        snapshot = {
            ...
            "bybit_adapter": {
                "collector_parser_version": "0.1.4",
                "parser_compatible": by_parser["parser_compatible"],
                "withdrawMax_minus_one_semantics": by_parser["withdrawMax_minus_one_semantics"],
                "parsed_chain_rows": by_parser["parsed_chain_rows"],
                "raw_withdrawMax_minus_one_rows": by_parser["raw_withdrawMax_minus_one_rows"],
                "normalized_unlimited_rows": by_parser["normalized_unlimited_rows"],
            },
            "okx_adapter": {
        ...
        print("Bybit parser v0.1.4 compatible =", by_parser["parser_compatible"])
        print("Bybit withdrawMax -1 rows =", by_parser["raw_withdrawMax_minus_one_rows"])
        print("Bybit normalized UNLIMITED rows =", by_parser["normalized_unlimited_rows"])
