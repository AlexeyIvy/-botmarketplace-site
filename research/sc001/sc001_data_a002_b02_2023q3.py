"""SC001-DATA-A002-B02 — frozen Binance BTCUSDT aggTrades 2023-Q3.
...
WORKSPACE = Path("/storage/emulated/0/Download/SC001_DATA_A002_B02_2023Q3")
...
EXPECTED_BATCH_COMPRESSED_BYTES = 53_592_159
EXPECTED_DAYS = 5
...
EXPECTED = [
    (
        "2023-07-12", "EVENT", "CPI", 15_117_699,
        "e19d5baa4d38b25d176ffb8e45e3e18812a1c19659d834827a0cd3ffdf60ab6e",
    ),
    (
        "2023-07-26", "EVENT", "FOMC", 10_484_375,
        "c7d43ced9bda96ed2d22eb9b783f9150df85917187a0b65e6b399e00a51426f4",
    ),
    (
        "2023-07-30", "ORDINARY_WEEKEND", None, 6_147_333,
        "8de5561586e4e554f6dd53e5d3c4882468ac070d32e885152743253d1723ec67",
    ),
    (
        "2023-08-04", "EVENT", "NFP", 7_865_401,
        "edd1fb4a9fab76814bc3523cc8d022feea23d84340b3dd5fedbbf93e2a72bff8",
    ),
    (
        "2023-08-23", "ORDINARY_WEEKDAY", None, 13_977_351,
        "d75b0426082a4be111138bb36db5e21b73916368245de379dd375834594dabe8",
    ),
]
...
def read_small(url: str) -> bytes:
    ...
    req = Request(
        url,
        headers={"User-Agent": "BotMarketplace-SC001-A002-B02/0.1"},
    )
...
def write_outputs(report: dict) -> None:
    atomic_json(WORKSPACE / "sc001_data_a002_b02_report.json", report)
    ...
    atomic_json(WORKSPACE / "sc001_data_a002_b02_manifest.json", manifest)

    lines = [
        "# SC001-DATA-A002-B02 — Binance BTCUSDT aggTrades 2023-Q3",
        "",
    ...
    lines += [
        ...
        "Under the A002 Development Firewall, B02 is DEV-DISCOVERY acquisition only.",
    ]
    (WORKSPACE / "sc001_data_a002_b02_summary.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )

    atomic_json(
        WORKSPACE / "sc001_data_a002_b02_final_safety.json",
        {
            "stage": STAGE,
...
def main() -> None:
    ...
    print(
        ...
        f"network={network_bytes_read} | P&L=NO | FEATURES=NO"
    )
