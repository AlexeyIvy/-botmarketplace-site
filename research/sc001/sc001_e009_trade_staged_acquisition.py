...
import argparse,csv,hashlib,io,json,os,shutil,zipfile
from pathlib import Path
from urllib.parse import urlparse
from urllib.request import Request,urlopen

DATA_ROOT=Path(os.environ.get('SC001_DATA_ROOT',str(Path.home()/'sc001_data'))).expanduser().resolve()
PARENT=DATA_ROOT/'SC001_E009_TRADE_METADATA_PREFLIGHT'/'sc001_e009_trade_metadata_preflight_report.json'
ROOT=DATA_ROOT/'SC001_E009_TRADE_ACQUISITION'; ARCH=ROOT/'archives'; REPORTS=ROOT/'reports'
UA='BotMarketplace-SC001-E009-Acquisition/0.1'; REFERER='https://www.okx.com/historical-data'; HOST='static.okx.com'
HEADER=['instrument_name','trade_id','side','price','size','created_time']
BATCHES={'a':('BTC','ETH'),'b':('DOGE','ORDI'),'c':('UNI','XRP'),'d':('OP','BCH')}
TOKENS={k:f'E009_TRADE_ACQUISITION_BATCH_{k.upper()}_PASS' for k in BATCHES}
VERIFY='E009_TRADE_ACQUISITION_VERIFY_PASS'; MIN_FREE=5_000_000_000
EXPECTED_FILES=128; EXPECTED_TOTAL_BYTES=419_552_195
...
def trusted(url,fn):
    u=urlparse(url)
    return u.scheme=='https' and (u.hostname or '').lower()==HOST and Path(u.path).name==fn
...
def parent_rows():
    p=load(PARENT)
    if p.get('status')!='E009_TRADE_METADATA_PREFLIGHT_PASS': fail('parent not PASS')
    rows=p.get('rows') or []
    if len(rows)!=EXPECTED_FILES or int(p.get('resolved_files',0))!=EXPECTED_FILES: fail('parent row count mismatch')
    if int(p.get('expected_total_bytes',-1))!=EXPECTED_TOTAL_BYTES: fail('parent total byte mismatch')
    if p.get('asset_holdout_accessed') is not False or p.get('october_confirmation_accessed') is not False or p.get('august_repurposed') is not False: fail('parent firewall mismatch')
    return p,rows
    ...
    with urlopen(req,timeout=120) as resp:
        ...
        with tmp.open('wb') as f:
            while True:
                ...
                if not b: break
                f.write(b)
    if not tmp.exists() or tmp.stat().st_size!=expected:
        got=tmp.stat().st_size if tmp.exists() else -1
        tmp.unlink(missing_ok=True); fail(f'size mismatch {fn}: {got}!={expected}')
...
def batch(name):
    ...
    expected=sum(int(r['bytes']) for r in selected); free=shutil.disk_usage(DATA_ROOT).free
    ...
    rep={'stage':'SC001-E009-TRADE-ACQUISITION','batch':name.upper(),'status':TOKENS[name],'files':out,'verified_files':len(out),'total_bytes':sum(x['local_bytes'] for x in out),'asset_holdout_accessed':False,'october_confirmation_accessed':False,'august_repurposed':False,'strategy_signal_calculated':False,'strategy_pnl_calculated':False}
...
def verify():
    ...
    for b in BATCHES:
        x=load(REPORTS/f'batch_{b}.json')
    ...
    if len(acquired)!=EXPECTED_FILES or set(acquired)!=set(byfn): fail('acquired file identity mismatch')
    total=0
    ...
    if total!=EXPECTED_TOTAL_BYTES or total!=int(parent['expected_total_bytes']): fail(f'aggregate bytes mismatch {total}')
    rep={'stage':'SC001-E009-TRADE-ACQUISITION-VERIFY','status':VERIFY,'verified_files':EXPECTED_FILES,'verified_total_bytes':total,'expected_total_bytes':EXPECTED_TOTAL_BYTES,'asset_holdout_accessed':False,'october_confirmation_accessed':False,'august_repurposed':False,'strategy_signal_calculated':False,'strategy_pnl_calculated':False}
    atomic(ROOT/'sc001_e009_trade_acquisition_verify_report.json',rep)
    print(VERIFY); print('verified_files = 128'); print('verified_total_bytes =',total); print('asset holdout accessed = False'); print('October Confirmation accessed = False'); print('August repurposed = False'); print('strategy signal/PnL calculated = False'); return 0

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('mode',choices=['batch-a','batch-b','batch-c','batch-d','verify']); a=ap.parse_args()
    return verify() if a.mode=='verify' else batch(a.mode[-1])
