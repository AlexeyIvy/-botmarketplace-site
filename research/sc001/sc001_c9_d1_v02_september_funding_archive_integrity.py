...
STAGE = "SC001-C9-D1-V0.2-SEPTEMBER-FUNDING-ARCHIVE-INTEGRITY"
PASS = "C9_D1_V02_FUNDING_ARCHIVE_INTEGRITY_PASS"
REVIEW = "C9_D1_V02_FUNDING_ARCHIVE_INTEGRITY_REVIEW"
...
OUT_DIR = DATA_ROOT / "SC001_C9_D1_V02_FUNDING_ARCHIVE"
ARCHIVE_DIR = DATA_ROOT / "SC001_C9_D1_FUNDING_ARCHIVE" / "archives"
OUT = OUT_DIR / "sc001_c9_d1_v02_funding_archive_integrity_report_v0_1.json"
...
def require_freeze() -> dict:
    fr = load_json(FREEZE)
    if fr.get("status") != "FROZEN_BEFORE_C9_D1_V02_RUN":
        fail("C9-D1 v0.2 freeze status mismatch")
...
def qualify_instrument(inst: str, parent: dict) -> dict:
    ...
    for ts, rate in combined:
        if ts < SOURCE_START_MS or ts > SOURCE_END_MS:
            fail(f"target funding timestamp outside authorized source-month boundary {inst}: {ts}")
        prev = by_ts.get(ts)
