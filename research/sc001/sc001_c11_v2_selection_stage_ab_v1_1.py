...
STAGE="SC001-C11-V2-SELECTION-STAGE-AB-V1.1"
...
PROTOCOL=ROOT/"docs/research/sc001-c11-direction-rule-v2-stage-ab-selection-protocol-v1.1.md"
REGISTRY=ROOT/"docs/research/sc001-contamination-registry-v0.20.json"
FREEZE=ROOT/"docs/research/sc001-c11-v2-stage-ab-selection-implementation-freeze-v1.1.json"
META_RESULT=ROOT/"docs/research/sc001-c11-v2-selection-metadata-preflight-pass-result-v0.3.md"
SCHEMA_REVIEW=ROOT/"docs/research/sc001-c11-v2-2026-trade-archive-schema-drift-review-v0.1.md"
...
OUT=OUT_DIR/"sc001_c11_v2_selection_stage_ab_report_v1_1.json"
...
def require():
    fr=load_json(FREEZE)
    if fr.get("status")!="FROZEN_BEFORE_C11_V2_SELECTION_STAGE_AB_V11_RUN": fail("freeze status mismatch")
    expected={
        ...
        "schema_review_git_blob_sha":git_blob(SCHEMA_REVIEW),
    }
    ...
    for dom in OKX_DOMAINS:
        try:
            req=urllib.request.Request(dom+"/priapi/v5/broker/public/trade-data/download-link?t="+str(int(time.time()*1000)),
                data=body,method="POST",headers={"User-Agent":"BotMarketplace-SC001-C11-V2-AB/1.1","Accept":"application/json,*/*","Content-Type":"application/json","Referer":"https://www.okx.com/historical-data"})
...
def head(u,b):
    req=urllib.request.Request(u,method="HEAD",headers={"User-Agent":"BotMarketplace-SC001-C11-V2-AB/1.1","Referer":"https://www.okx.com/historical-data"})
    with urllib.request.urlopen(req,timeout=TIMEOUT) as resp:
...
def download(u,b,size):
    ...
    got=0
    req=urllib.request.Request(u,headers={"User-Agent":"BotMarketplace-SC001-C11-V2-AB/1.1","Referer":"https://www.okx.com/historical-data"})
...
def anchors(path,event_us):
    ...
    schema=None; expected_len=None; source0=source1=None
    ...
    with zipfile.ZipFile(path) as z:
        ...
        with z.open(mem[0]) as raw:
            ...
            header=next(r,None)
            if header==HEADER6:
                schema="LEGACY_6"; expected_len=6; source0=source1=None
            elif header==HEADER7:
                schema="SOURCE_7"; expected_len=7; source0=0; source1=0
            else:
                fail(f"unrecognized header: {header}")

            for row in r:
                if not row: continue
                if len(row)!=expected_len: fail(f"row field-count mismatch schema={schema}")
                if row[0]!=INST or row[2] not in {"buy","sell"}: fail("row semantics")
                if schema=="SOURCE_7":
                    src=row[6]
                    if src not in SOURCE_ALLOWED: fail(f"unknown source value: {src!r}")
                    if src=="0": source0+=1
                    else: source1+=1
    ...
    diag={"trade_schema":schema,"source_0_rows":source0,"source_1_rows":source1}
    if None in (pre,p1,p60,tpre,t1,t60):
        return {"valid":False,"reason":"MISSING_ANCHOR",**diag}
    if event_us-tpre>1_000_000:
        return {"valid":False,"reason":"PRE_STALE",**diag}
    if target1-t1>1_000_000:
        return {"valid":False,"reason":"PLUS1_STALE",**diag}
    if target60-t60>1_000_000:
        return {"valid":False,"reason":"PLUS60_STALE",**diag}
    return {"valid":True,"pre_price":pre,"price_1s":p1,"price_60s":p60,
            ...
            "plus60_staleness_ms":(target60-t60)/1000.0,**diag}
...
def main():
    try:
        ...
        for i,e in enumerate(meta["events"],1):
            ...
            print(f"C11-V2-AB-V11 [{i}/24] {kind} {date} metadata/body-check",flush=True)
            ...
            row={
                "kind":kind,"date":date,"event_utc":utc,"archive":b,"bytes":size,"reused":reused,
                "trade_schema":a["trade_schema"],
                "source_0_rows":a["source_0_rows"],
                "source_1_rows":a["source_1_rows"],
                "anchor_valid":bool(a["valid"]),
                "data_invalid_reason":None if a["valid"] else a["reason"],
            }
            if a["valid"]:
        ...
        rep={
            "stage":STAGE,"version":"1.1","status":status,
            "scheduled_events":24,
            ...
            "schema_counts":{
                "LEGACY_6":sum(1 for r in rows if r["trade_schema"]=="LEGACY_6"),
                "SOURCE_7":sum(1 for r in rows if r["trade_schema"]=="SOURCE_7"),
            },
            "stage_a":stage_a,
            "stage_b":stage_b,
            "events":rows,
            "source_field_used_in_signal":False,
        ...
        print("schema_counts =",rep["schema_counts"])
        print("source_field_used_in_signal = False")
