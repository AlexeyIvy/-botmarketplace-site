...
STAGE = "SC001-C11-V2-SC-METADATA-PREFLIGHT-V0.3"
...
PROTOCOL = ROOT / "docs/research/sc001-c11-v2-selection-metadata-preflight-protocol-v0.3.md"
FREEZE = ROOT / "docs/research/sc001-c11-v2-selection-metadata-preflight-implementation-freeze-v0.3.json"
...
OUT = OUT_DIR / "sc001_c11_v2_sc_metadata_preflight_report_v0_3.json"
...
def require_freeze() -> None:
    fr = load_json(FREEZE)
    if fr.get("status") != "FROZEN_BEFORE_C11_V2_SC_METADATA_V03_RUN":
...
def bls_event_pattern(date: str, title: str) -> re.Pattern:
...
def verify_bls(kind: str, date: str, url: str, title: str) -> dict:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "BotMarketplace-SC001-C11-V2-Metadata/0.3",
            "Accept": "text/html,*/*",
        },
    )
    ...
    match = bls_event_pattern(date, title).search(text)
    if match is None:
        fail(f"BLS visible-text metadata mismatch {kind} {date}")

    return {
        "pass": True,
        "url": url,
        "final_url": final,
        "matched_visible_text": match.group(0),
...
def main() -> int:
    try:
        ...
        for i, (kind, date, utc, url, title) in enumerate(EVENTS, 1):
            print(f"C11-V2-SC-META-V03 [{i}/24] {kind} {date}", flush=True)
        ...
        report = {
            ...
            "v02_review_reason": "ENGINEERING_BLS_RAW_HTML_REGEX_DEFECT",
