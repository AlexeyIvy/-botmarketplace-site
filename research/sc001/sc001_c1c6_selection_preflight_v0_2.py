...
STAGE = "SC001-C1C6-SELECTION-PREFLIGHT-V0.2"
...
OUT = OUT_DIR / "sc001_c1c6_selection_preflight_report_v0_2.json"

RUNNER = ROOT / "research/sc001/sc001_c1c6_selection_preflight_v0_2.py"
PROTOCOL = ROOT / "docs/research/sc001-c1-c6-selection-preflight-protocol-v0.2.md"
FREEZE = ROOT / "docs/research/sc001-c1-c6-selection-preflight-implementation-freeze-v1.1.json"
...
def check_identity_freeze() -> dict:
    fr = load_json(FREEZE)
    if fr.get("status") != "FROZEN_AFTER_V0_1_FALSE_POSITIVE_BEFORE_SENTINEL_OUTCOME":
...
def verify_optional_legacy_e006() -> dict:
    ...
    out = {
        ...
        "ready": False,
        "warnings": [],
        "opened_date_range": "2024-03-01..2024-03-21 only when verifying archives",
    }
    ...
    try:
        ...
        def verify_rows(root: Path, rows: dict[str, dict]) -> int:
            n = 0
        ...
        out["spot_archives_verified"] = verify_rows(E006_SPOT, spot_rows)
        out["swap_archives_verified"] = verify_rows(E006_SWAP, swap_rows)
        out["ready"] = True
...
def scan_protected_bodies(files: list[Path]) -> tuple[list[str], list[str]]:
    ...
    for p in files:
        ...
        if path_has_any_date(text, march_confirm):
            if "SC001_E006_SPOT_FEASIBILITY" in text:
                hard_hits.append(str(p))
                continue
            if "SC001_E003_OKX_MARCH_TRADES" in text:
                legacy_e003_presence.append(str(p))
                continue

    return sorted(set(hard_hits)), sorted(set(legacy_e003_presence))
...
def run_preflight() -> dict:
    ...
    files = body_files()
    hard_hits, legacy_e003_presence = scan_protected_bodies(files)
    if hard_hits:
        fail(f"hard protected market-body files detected: {hard_hits[:5]}{' ...' if len(hard_hits)>5 else ''}")
    ...
    rep = {
        ...
        **identity,
        ...
        "hard_protected_market_body_hits": [],
        "hard_protected_market_body_hit_count": 0,
        "legacy_e003_march_confirmation_source_presence_only": legacy_e003_presence,
        "legacy_e003_march_confirmation_source_presence_count": len(legacy_e003_presence),
        "market_body_files_scanned_by_name_only": len(files),
        ...
        "legacy_e003_march_22_30_bodies_opened_or_hashed_by_this_preflight": False,
    }
    atomic_json(OUT, rep)
    return rep
...
def main() -> int:
    ...
    print("hard protected market-body hits = 0")
    print("legacy E003 March 22-30 source presence only =", rep["legacy_e003_march_confirmation_source_presence_count"])
    print("legacy E003 March 22-30 bodies opened/hashed by this preflight = False")
