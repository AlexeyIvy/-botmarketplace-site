...
STAGE = "SC001-B15P1-NONPRICE-SOURCE-CAPABILITY-REVALIDATION-V0.1"
PASS = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_PASS"
REVIEW = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_REVIEW"
SELFTEST_PASS = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_SELF_TEST_PASS"
SELFTEST_REVIEW = "B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_SELF_TEST_REVIEW"
...
PROTOCOL = ROOT / "docs/research/sc001-b15-p1-nonprice-source-capability-revalidation-v0.1.md"
SPEC = ROOT / "docs/research/sc001-b15-p1-nonprice-source-capability-revalidation-spec-v0.1.json"
FREEZE = ROOT / "docs/research/sc001-b15-p1-nonprice-source-capability-revalidation-freeze-v0.1.json"

RUNNER = ROOT / "research/sc001/sc001_b15p1_nonprice_transferability_collector_v0_1_2.py"
IMPLEMENTATION_FREEZE = ROOT / "docs/research/sc001-b15-p1-nonprice-collector-implementation-freeze-v0.1.2.json"
SERVICE = ROOT / "ops/systemd/sc001-b15p1-transferability-v0.1.2.service"
...
IMPLEMENTATION_SELFTEST = ROOT / "docs/research/artifacts/b15-p1-collector-implementation-v0.1.2/20260924T120400Z/implementation_self_test_manifest.json"
...
def require_freeze() -> dict[str, Any]:
    ...
    if fr.get("status") != "FROZEN_BEFORE_B15P1_NONPRICE_SOURCE_CAPABILITY_REVALIDATION_V01":
    ...
    expected = {
        ...
        "offline_self_test_manifest_sha256": sha256_file(IMPLEMENTATION_SELFTEST),
    }
...
def validate_okx_currencies(obj: dict[str, Any]) -> dict[str, int]:
    if str(obj.get("code")) != "0":
    ...
    return {"currency_chain_rows": len(rows)}
...
def bybit_live_usdt_spot_pairs(base: str) -> dict[str, str]:
    ...
    for page in range(10):
        ...
        if not cursor:
            return mapping
    fail("Bybit instrument pagination exceeded 10 pages")


def okx_live_usdt_spot_pairs(base: str) -> dict[str, str]:
    ...
    for row in rows:
        ...
        if not asset or not inst:
            continue
        if asset in mapping and mapping[asset] != inst:
...
def okx_fee_probe(
    ...
    row = rows[0]
    if row.get("taker") in (None, "") or row.get("maker") in (None, ""):
        fail("OKX fee fields missing")
    return {
        "instrument": inst_id,
        "taker_fee_field_present": True,
        "maker_fee_field_present": True,
        "fee_group_present": bool(row.get("feeGroup") or row.get("groupId")),
    }
...
def build_snapshot_fixture(
    ...
    return {
        "schema": "sc001.b15.p1_nonprice_source_capability_snapshot.v0.1",
        "status": PASS,
...
def write_selftest_manifest(status: str, error: str | None = None) -> None:
    ...
    atomic_json(
        ...
        {
            "schema": "sc001.b15.p1_nonprice_source_capability_revalidation_self_test.v0.1",
            "status": status,
            "error": error,
...
def live_run() -> int:
    try:
        ...
        safe_summary = {
            "schema": "sc001.b15.p1_nonprice_source_capability_safe_summary.v0.1",
            "status": PASS,
