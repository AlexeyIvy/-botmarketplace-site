"""SC001-DATA-A002-B04B05 — DEV-CONFIRMATION acquisition only.

Downloads/qualifies the 10 frozen Binance BTCUSDT USD-M aggTrades days.
No E002 features, no return labels, no P&L, no Validation/Final access.
...
WORKSPACE = Path("/storage/emulated/0/Download/SC001_DATA_A002_B04B05_CONFIRMATION")
ARCHIVES = WORKSPACE / "archives"

SESSION_DOWNLOAD_CAP_BYTES = 300_000_000
WORKSPACE_CAP_BYTES = 300_000_000
...
EXPECTED_TOTAL_COMPRESSED_BYTES = 194_145_949
EXPECTED_DAYS = 10
...
# date, quarter, sample_type, event_class, bytes, sha256
EXPECTED = [
    ("2024-01-05", "2024-Q1", "EVENT", "NFP", 20_807_819, "54cfc506daff386c40ac923d161cac31f7c53d55c412165c7daefdca1d428416"),
    ("2024-01-14", "2024-Q1", "ORDINARY_WEEKEND", None, 10_575_353, "60aef45bc81962b230f41fd41c1ff1bddaa69893a3d93f61b77c85b0cca29c32"),
    ("2024-01-31", "2024-Q1", "EVENT", "FOMC", 15_284_170, "b4748dc7abaaf3cd3b4f2c6c22ca68e32273760e47a85d0ef81cda9272e8b4f9"),
    ("2024-02-12", "2024-Q1", "ORDINARY_WEEKDAY", None, 22_088_730, "79fcbd8c45cb95fc966234d856579174b89002c7e6105fa191333c50b7aa35ae"),
    ("2024-02-13", "2024-Q1", "EVENT", "CPI", 19_099_037, "625d14e8a257fd9af2ff20cb81a4583191817bceb601133e04740d1ecaafb0fc"),
    ("2024-04-10", "2024-Q2", "EVENT", "CPI", 23_733_631, "d07810250019fec4f9201eddb9fbc68a1ec4e9b1854af9921e0e5b0154e8c388"),
    ("2024-04-20", "2024-Q2", "ORDINARY_WEEKEND", None, 16_313_893, "f00b085f8262e0a4d5fd97d4c751344877d4194ac0704724c7d21f15024764ba"),
    ("2024-05-01", "2024-Q2", "EVENT", "FOMC", 38_513_458, "f41f292348137f14cfc888ba458fabbaa7db95a871c45191c42c74993cce8c4a"),
    ("2024-06-07", "2024-Q2", "EVENT", "NFP", 17_788_758, "7ab0dbb225de014133a92b4cadefdba25f35faf7639fa54b26495b22fb4581ab"),
    ("2024-06-27", "2024-Q2", "ORDINARY_WEEKDAY", None, 9_941_100, "5be30ff49a4f6f5e52b971fdb5cf705ad963831b787f4d42aa5224fe6e33aa79"),
]
...
def read_small(url: str) -> bytes:
    ...
    req = Request(url, headers={"User-Agent": "BotMarketplace-SC001-A002-B04B05/0.1"})
...
def head_content_length(url: str) -> int:
    req = Request(url, method="HEAD", headers={"User-Agent": "BotMarketplace-SC001-A002-B04B05/0.1"})
    with urlopen(req, timeout=60) as resp:
...
def download_archive(url: str, dest: Path, expected_bytes: int, expected_sha: str) -> dict:
    ...
    if dest.exists():
        if dest.stat().st_size == expected_bytes and sha256_file(dest) == expected_sha:
            return {"status": "REUSED", "bytes": expected_bytes, "sha256": expected_sha, "network_bytes": 0}
        dest.unlink()
    ...
    try:
        ...
        return {"status": "DOWNLOADED", "bytes": got, "sha256": digest, "network_bytes": got}
    except Exception:
        if part.exists():
            part.unlink()
        raise
...
def validate_zip(path: Path, date_text: str) -> dict:
    ...
    with zipfile.ZipFile(path, "r") as zf:
        ...
        rows = invalid_rows = out_of_day = 0
        timestamp_backwards = same_ms_adjacent = 0
        agg_id_duplicate_or_backwards = agg_id_gap_count = 0
        underlying_overlap_or_backwards = underlying_gap_count_diagnostic = 0
        maker_true_rows = maker_false_rows = 0
        first_agg_id = last_agg_id = None
        first_underlying_id = last_underlying_id = None
        first_ts = last_ts = None
        previous_agg_id = previous_underlying_last = previous_ts = None
        minute_buckets = set()
        header_mode = None
            ...
            for row in reader:
                ...
                if not first_nonempty_seen:
                    try:
                        ...
                        if [x.strip().lower() for x in row] != HEADER:
    ...
    gates = {
        ...
        "agg_trade_ids_strict_contiguous": agg_id_duplicate_or_backwards == 0 and agg_id_gap_count == 0,
...
def verify_preflight() -> dict:
    ...
    if obj.get("stage") != PREFLIGHT_STAGE or obj.get("overall_status") != "PASS":
        raise RuntimeError("preflight stage/status mismatch")
    if obj.get("strategy_pnl_calculated") is not False or obj.get("archive_bodies_downloaded") is not False:
        raise RuntimeError("preflight boundary mismatch")
    if scope.get("frozen_calendar_sha256") != FROZEN_CALENDAR_SHA256 or scope.get("split") != "DEV_ONLY":
        raise RuntimeError("preflight scope/calendar mismatch")
    ...
    for date_text, _, _, _, expected_bytes, expected_sha in EXPECTED:
        ...
        if row.get("expected_sha256") != expected_sha:
            raise RuntimeError(f"preflight SHA mismatch: {date_text}")
        if (row.get("archive_metadata") or {}).get("content_length") != expected_bytes:
            raise RuntimeError(f"preflight size mismatch: {date_text}")
    return {
...
def write_outputs(report: dict) -> None:
    atomic_json(WORKSPACE / "sc001_data_a002_b04b05_report.json", report)
    manifest = {
        "stage": STAGE,
        "version": VERSION,
    ...
    atomic_json(WORKSPACE / "sc001_data_a002_b04b05_manifest.json", manifest)

    lines = [
        "# SC001-DATA-A002-B04B05 — DEV-CONFIRMATION acquisition",
        "",
        f"- Overall: `{report.get('overall_status')}`",
        "- Strategy features calculated: **NO**",
        "- Strategy/P&L calculated: **NO**",
        "- Confirmation metrics calculated: **NO**",
    ...
    for row in report.get("days", []):
        val = row["validation"]
        tag = row["sample_type"] + (("/" + row["event_class"]) if row["event_class"] else "")
        lines.append(
            f"- {row['date']} [{row['quarter']} {tag}]: **{row['status']}**; "
            f"rows={val['rows']}; minutes={val['minute_buckets_observed']}/1440; "
            f"agg_id_gaps={val['agg_id_gap_count']}; timestamp_backwards={val['timestamp_backwards']}; "
    ...
    lines += [
        "",
        "## Boundary",
        "Data qualification only. Do not inspect E002 confirmation metrics until all 10 days pass and the frozen combined confirmation engine is run once.",
    ]
    (WORKSPACE / "sc001_data_a002_b04b05_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

    atomic_json(WORKSPACE / "sc001_data_a002_b04b05_final_safety.json", {
        "stage": STAGE,
...
def main() -> None:
    ...
    report = {
        ...
        "scope": {
            ...
            "symbol": SYMBOL,
            "dataset": DATASET,
            "split": "DEV_CONFIRMATION_ACQUISITION_ONLY",
            "quarters": ["2024-Q1", "2024-Q2"],
            "dates": [x[0] for x in EXPECTED],
            "frozen_calendar_sha256": FROZEN_CALENDAR_SHA256,
            "expected_compressed_bytes": EXPECTED_TOTAL_COMPRESSED_BYTES,
        },
    ...
    prev_selected_last_agg = None
    prev_selected_last_underlying = None
    cross_order_violations = []

    try:
        for date_text, quarter, sample_type, event_class, expected_bytes, expected_sha in EXPECTED:
            ...
            if prev_selected_last_agg is not None and validation["first_agg_trade_id"] <= prev_selected_last_agg:
                cross_order_violations.append({
                    "date": date_text,
                    "kind": "agg_trade_id_not_forward",
                    "previous_last": prev_selected_last_agg,
                    "current_first": validation["first_agg_trade_id"],
                })
            if prev_selected_last_underlying is not None and validation["first_underlying_trade_id"] <= prev_selected_last_underlying:
                cross_order_violations.append({
                    "date": date_text,
                    "kind": "underlying_trade_id_not_forward",
                    "previous_last": prev_selected_last_underlying,
                    "current_first": validation["first_underlying_trade_id"],
                })

            prev_selected_last_agg = validation["last_agg_trade_id"]
            prev_selected_last_underlying = validation["last_underlying_trade_id"]
            ...
            report["days_passed_so_far"] = sum(1 for x in report["days"] if x["status"] == "PASS")
        ...
        report["network_bytes_read"] = network_bytes_read
        report["overall_status"] = "PASS" if (
            report["days_total"] == EXPECTED_DAYS
    ...
    print(
        ...
        f"network={network_bytes_read} | FEATURES=NO | P&L=NO"
    )
