...
EXPECTED_TOKEN = "B15P1_NONPRICE_COLLECTOR_V014_SELF_TEST_PASS"
...
def main() -> int:
    ...
    try:
        ...
        compile_checks["bybit_withdraw_max_sentinel_guard"] = {
            "passed": (
                "bybit_withdraw_max_text" in LIBRARY.read_text(encoding="utf-8")
                and '"UNLIMITED"' in LIBRARY.read_text(encoding="utf-8")
                and 'withdrawMax' in LIBRARY.read_text(encoding="utf-8")
            ),
        }
        ...
        pass_ok = (
            ...
            and len(selftest_obj.get("mandatory_tests") or []) == 32
            and all(
                (selftest_obj.get("test_results") or {}).get(name) is True
                for name in (selftest_obj.get("mandatory_tests") or [])
            )
            ...
            and selftest_obj.get("pnl_data_authorized") is False
        )
        if not pass_ok:
            raise RuntimeError(
        ...
        write_json(
            COMPILE_MANIFEST,
            {
                "schema": "sc001.b15.p1_nonprice_collector_compile_guard.v0.1.4",
                "status": "B15P1_NONPRICE_COLLECTOR_V014_COMPILE_AND_SELFTEST_PASS",
        ...
        print("B15P1_NONPRICE_COLLECTOR_V014_COMPILE_AND_SELFTEST_PASS")
        return 0
        ...
        write_json(
            ...
            {
                "schema": "sc001.b15.p1_nonprice_collector_compile_guard.v0.1.4",
                "status": "B15P1_NONPRICE_COLLECTOR_V014_COMPILE_OR_SELFTEST_REVIEW",
                "compile_checks": compile_checks,
                "error": error,
                "exchange_calls_performed": False,
                "collector_launch_authorized": False,
        ...
        print("B15P1_NONPRICE_COLLECTOR_V014_COMPILE_OR_SELFTEST_REVIEW")
        print("error =", error)
        return 2
