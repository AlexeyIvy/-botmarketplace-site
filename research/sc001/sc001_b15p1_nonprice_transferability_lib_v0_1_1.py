    ...
    for coin_row in rows:
        ...
        for ch in chains:
            ...
            row = {
                ...
                "max_withdrawal": decimal_text(ch.get("withdrawMax")),
...
def parse_okx_currencies(obj: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for src in rows:
        if not isinstance(src, dict) or not required.issubset(src):
            raise ValueError("OKX currencies schema drift")
        ...
        burn = decimal_text(src.get("burningFeeRate"), empty_zero=True)
        fee_ccy = clean_text(src.get("feeCcy")) or asset
        row = {
            "venue": "OKX",
            "asset": asset,
            ...
            "confirmation_metadata": {
                "minDepArrivalConfirm": clean_text(src.get("minDepArrivalConfirm")) or None,
                "minWdUnlockConfirm": clean_text(src.get("minWdUnlockConfirm")) or None,
            },
            "metadata_valid": True,
        }
        if any(
