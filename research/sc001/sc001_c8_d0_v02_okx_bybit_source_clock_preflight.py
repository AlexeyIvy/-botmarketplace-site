...
STAGE = "SC001-C8-D0-V0.2-OKX-BYBIT-SOURCE-CLOCK-PREFLIGHT"
PASS = "C8_D0_V02_SOURCE_CLOCK_PREFLIGHT_PASS"
REVIEW = "C8_D0_V02_SOURCE_CLOCK_PREFLIGHT_REVIEW"
...
PROTOCOL = ROOT / "docs/research/sc001-c8-d0-okx-bybit-source-clock-semantics-protocol-v0.2.md"
FREEZE = ROOT / "docs/research/sc001-c8-d0-v0.2-implementation-freeze-v0.1.json"

DATA_ROOT = Path(
    os.environ.get("SC001_DATA_ROOT", str(Path.home() / "sc001_data"))
).expanduser().resolve()
OUT_DIR = DATA_ROOT / "SC001_C8_D0_V02_SOURCE_CLOCK"
OUT = OUT_DIR / "sc001_c8_d0_v02_source_clock_preflight_v0_1.json"
...
def require_freeze() -> dict:
    fr = load_json(FREEZE)
    if fr.get("status") != "FROZEN_BEFORE_C8_D0_V02_RUN":
    ...
    for attempt in range(1, RETRIES + 1):
        try:
            req = urllib.request.Request(
                ...
                method="GET",
                headers={
                    "User-Agent": "BotMarketplace-SC001-C8-D0-v0.2",
...
def okx_archive_metadata() -> dict:
    query_windows = (
        ("LABEL_DAY", DAY_START_MS, DAY_END_MS),
        ("PREVIOUS_DAY", PREV_DAY_START_MS, PREV_DAY_END_MS),
    )

    attempts: list[dict] = []

    for query_label, begin_ms, end_ms in query_windows:
        obj, final, nbytes = okx_json(
        ...
        attempts.append(
            {
                ...
                "metadata_response_bytes": nbytes,
                "final_host": urllib.parse.urlparse(final).hostname,
            }
        )
        ...
        if len(found) != 1:
            fail(
                f"OKX exact historical archive resolution count={len(found)} "
                f"query={query_label} expected={OKX_EXPECTED_ARCHIVE}"
            )
        ...
        return {
            ...
            "selected_query_label": query_label,
            "attempts": attempts,
            "pass": True,
        }

    fail(
        "OKX exact historical archive unresolved on frozen label-day "
        "and previous-day metadata windows "
