...
META = DATA_ROOT / 'SC001_E009_TRADE_METADATA_PREFLIGHT' / 'sc001_e009_trade_metadata_preflight_report.json'
ACQ = DATA_ROOT / 'SC001_E009_TRADE_ACQUISITION'
ARCH = ACQ / 'archives'
VERIFY = ACQ / 'sc001_e009_trade_acquisition_verify_report.json'
OUT_DIR = DATA_ROOT / 'SC001_E009_TRADE_SEMANTIC_INTEGRITY'
PREFLIGHT_REPORT = OUT_DIR / 'sc001_e009_trade_semantic_preflight_report.json'
REPORT = OUT_DIR / 'sc001_e009_trade_semantic_integrity_report.json'

ASSETS = ('BTC', 'ETH', 'DOGE', 'ORDI', 'UNI', 'XRP', 'OP', 'BCH')
ARCHIVE_DATES = tuple(['2024-08-31'] + [f'2024-09-{d:02d}' for d in range(1, 16)])
TARGET_DATES = tuple(['2024-08-31'] + [f'2024-09-{d:02d}' for d in range(1, 15)])
HEADER = ['instrument_name', 'trade_id', 'side', 'price', 'size', 'created_time']
PREFLIGHT_PASS = 'E009_TRADE_SEMANTIC_PREFLIGHT_PASS'
SEMANTIC_PASS = 'E009_TRADE_SEMANTIC_INTEGRITY_PASS'
EXPECTED_BYTES = 419552195
...
def parent_map() -> dict[str, dict]:
    meta = load(META)
    if meta.get('status') != 'E009_TRADE_METADATA_PREFLIGHT_PASS':
...
def preflight() -> int:
    ...
    rep = {
        'stage': 'SC001-E009-TRADE-SEMANTIC-PREFLIGHT',
...
def scan_source(path: Path, inst: str) -> dict:
    ...
    try:
        for row in reader:
            if not row:
                continue
            tid, ts, _side, _price, _size = parse_row(row, inst, path.name)
...
def iter_target_rows(path: Path, inst: str, lo: int, hi: int):
    ...
    try:
        for row in reader:
            if not row:
                continue
            tid, ts, side, _price, _size = parse_row(row, inst, path.name)
            if lo <= ts < hi:
                yield tid, ts, side
    finally:
        text.close(); zf.close()
...
def target_day(sym: str, day: str, byfn: dict[str, dict]) -> dict:
    ...
    return {
        ...
        'both_sides': True,
        'performance_role': 'BOUNDARY_WARMUP' if day == '2024-08-31' else 'DISCOVERY_PERFORMANCE',
    }
...
def run() -> int:
    ...
    rep = {
        'stage': 'SC001-E009-TRADE-SEMANTIC-INTEGRITY',
        'protocol_version': '0.1',
        ...
        'sparse_days': [
            {
                'instrument': r['instrument'], 'date': r['date'],
