...
from sc001_b15p1_nonprice_transferability_lib_v0_1 import (
...
STAGE = "SC001-B15P1-NONPRICE-TRANSFERABILITY-COLLECTOR-V0.1"
VERSION = "0.1"
SELFTEST_PASS = "B15P1_NONPRICE_COLLECTOR_V01_SELF_TEST_PASS"
SELFTEST_REVIEW = "B15P1_NONPRICE_COLLECTOR_V01_SELF_TEST_REVIEW"
...
IMPLEMENTATION_SEMANTICS = ROOT / "docs/research/sc001-b15-p1-nonprice-collector-implementation-semantics-v0.1.md"
RAW_STORAGE_AMENDMENT = ROOT / "docs/research/sc001-b15-p1-collector-raw-storage-amendment-v0.1.md"
IMPLEMENTATION_CONTRACT = ROOT / "docs/research/artifacts/b15-p1-collector-design/20260924T103023Z/collector_implementation_contract_candidate.json"
...
SERVICE_FILE = ROOT / "ops/systemd/sc001-b15p1-transferability.service"
FREEZE = ROOT / "docs/research/sc001-b15-p1-nonprice-collector-implementation-freeze-v0.1.json"
...
def require_freeze() -> dict[str, Any]:
    ...
    if fr.get("status") != "FROZEN_BEFORE_B15P1_NONPRICE_COLLECTOR_SELF_TEST_V01":
        fail("implementation freeze status mismatch")
    expected = {
        "runner_sha256": sha256_file(Path(__file__).resolve()),
        "library_sha256": sha256_file(
            Path(__file__).resolve().with_name(
                "sc001_b15p1_nonprice_transferability_lib_v0_1.py"
            )
        ),
...
def write_selftest_report(
    ...
    *,
    ...
    obj = {
        "schema": "sc001.b15.p1_nonprice_collector_implementation_self_test.v0.1",
        "status": status,
...
def self_test() -> int:
    ...
    try:
        ...
        from sc001_b15p1_nonprice_transferability_lib_v0_1 import validate_capability_snapshot
        validate_capability_snapshot(
            cap,
            runner_sha256=runner_sha,
...
def parse_fee_result(item: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if venue == "BYBIT":
            ...
            r = rows[0]
            row["taker_fee_rate"] = str(r.get("taker"))
            row["maker_fee_rate"] = str(r.get("maker"))
            row["account_fee_group"] = r.get("feeGroup") or r.get("groupId")
        row["ok"] = True
        row["error"] = None
    except Exception as exc:
...
def parse_fast_result(
    ...
    if not result.get("ok"):
        return False, [], result.get("error") or "HTTP_ERROR"
    try:
        ...
        while not STOP_REQUESTED:
            ...
            for venue, fut in submitted:
                ...
                try:
                    ...
                    ok, rows, error = parse_fast_result(venue, result)
