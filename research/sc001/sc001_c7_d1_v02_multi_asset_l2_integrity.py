...
STAGE = "SC001-C7-D1-V0.2-MULTI-ASSET-L2-INTEGRITY-NORMALIZATION"
PASS = "C7_D1_V02_MULTI_ASSET_L2_INTEGRITY_PASS"
REVIEW = "C7_D1_V02_MULTI_ASSET_L2_INTEGRITY_REVIEW"
...
DOGE = "DOGE-USDT-SWAP"
INHERITED = tuple(x for x in INSTS if x != DOGE)
...
PROTOCOL = ROOT / "docs/research/sc001-c7-d1-multi-asset-l2-body-integrity-normalization-v0.2.md"
REGISTRY = ROOT / "docs/research/sc001-contamination-registry-v0.14.json"
FREEZE = ROOT / "docs/research/sc001-c7-d1-v0.2-implementation-freeze-v0.1.json"
...
V01_ROOT = DATA_ROOT / "SC001_C7_D1_MULTI_ASSET_L2"
V01_REPORT = V01_ROOT / "sc001_c7_d1_multi_asset_l2_integrity_report_v0_1.json"
V01_ARCHIVES = V01_ROOT / "archives"

OUT_DIR = DATA_ROOT / "SC001_C7_D1_V02_MULTI_ASSET_L2"
NORMALIZED_DIR = OUT_DIR / "normalized"
OUT = OUT_DIR / "sc001_c7_d1_v02_multi_asset_l2_integrity_report_v0_1.json"
...
def require_freeze() -> dict:
    fr = load_json(FREEZE)
    if fr.get("status") != "FROZEN_BEFORE_C7_D1_V02_RUN":
        fail("C7-D1 v0.2 freeze status mismatch")
    if fr.get("runner_git_blob_sha") != git_blob(Path(__file__).resolve()):
        fail("C7-D1 v0.2 runner identity mismatch")
    if fr.get("protocol_git_blob_sha") != git_blob(PROTOCOL):
        fail("C7-D1 v0.2 protocol identity mismatch")
    if fr.get("contamination_registry_git_blob_sha") != git_blob(REGISTRY):
        fail("C7-D1 v0.2 registry identity mismatch")
    if tuple(fr.get("universe") or ()) != INSTS:
        fail("C7-D1 v0.2 universe mismatch")
    if fr.get("repair_rule") != "discard_leading_updates_until_first_snapshot":
        fail("C7-D1 v0.2 repair rule mismatch")
    if fr.get("fixed_date") != FIXED_DATE:
        fail("C7-D1 v0.2 date mismatch")
    for k in (
        ...
        if fr.get(k) is not False:
            fail(f"C7-D1 v0.2 firewall mismatch: {k}")
    return fr
...
def require_d0() -> dict:
    ...
    if int(rep.get("assets_passed", 0)) != len(INSTS):
        fail("C7-D0 parent assets mismatch")
    return rep


def require_v01_parent() -> dict:
    rep = load_json(V01_REPORT)
    if rep.get("status") != "C7_D1_MULTI_ASSET_L2_INTEGRITY_REVIEW":
        fail("C7-D1 v0.1 parent status mismatch")
    if int(rep.get("assets_passed", 0)) != 6:
        fail("C7-D1 v0.1 parent assets_passed mismatch")

    assets = rep.get("assets") or {}
    failed = [
        inst for inst in INSTS
        if (assets.get(inst) or {}).get("integrity_pass") is not True
    ]
    if failed != [DOGE]:
        fail(f"C7-D1 v0.1 failed asset set mismatch: {failed}")

    err = str((assets.get(DOGE) or {}).get("error", ""))
    if "first action not snapshot" not in err:
        fail(f"C7-D1 v0.1 DOGE error mismatch: {err}")
    ...
    for k in (
        ...
        if rep.get(k) is not False:
            fail(f"C7-D1 v0.1 parent firewall mismatch: {k}")
    return rep


def verify_inherited_assets(parent: dict) -> dict:
    verified = {}
    assets = parent.get("assets") or {}

    for inst in INHERITED:
        row = assets.get(inst)
        if not isinstance(row, dict) or row.get("integrity_pass") is not True:
            fail(f"inherited asset not PASS in v0.1: {inst}")

        src = row.get("source_archive") or {}
        norm = row.get("normalization") or {}
        norm_meta = norm.get("normalized_file") or {}

        src_path = V01_ARCHIVES / str(src.get("filename", ""))
        norm_path = Path(str(norm_meta.get("path", "")))

        if not src_path.exists():
            fail(f"inherited source missing: {inst} {src_path}")
        if src_path.stat().st_size != int(src.get("bytes", -1)):
            fail(f"inherited source byte mismatch: {inst}")
        if sha256_file(src_path) != src.get("sha256"):
            fail(f"inherited source SHA mismatch: {inst}")

        if not norm_path.exists():
            fail(f"inherited normalized file missing: {inst} {norm_path}")
        if norm_path.stat().st_size != int(norm_meta.get("bytes", -1)):
            fail(f"inherited normalized byte mismatch: {inst}")
        if sha256_file(norm_path) != norm_meta.get("sha256"):
            fail(f"inherited normalized SHA mismatch: {inst}")

        if int(norm.get("valid_sampled_seconds", 0)) < 80_000:
            fail(f"inherited valid-second gate mismatch: {inst}")
        if int(norm.get("active_utc_hours", 0)) != 24:
            fail(f"inherited active-hour gate mismatch: {inst}")

        verified[inst] = {
            "integrity_pass": True,
            "inherited_from_v01": True,
            "source_archive": src,
            "normalization": norm,
        }
    ...
    for domain in OKX_DOMAINS:
        ...
        try:
            req = urllib.request.Request(
                ...
                method="POST",
                headers={
                    "User-Agent": "BotMarketplace-SC001-C7-D1-v0.2",
...
def download_exact(
    ...
    V01_ARCHIVES.mkdir(parents=True, exist_ok=True)
    dest = V01_ARCHIVES / expected_basename
...
def iter_l2_repaired(path: Path, inst: str, diag: dict):
    with tarfile.open(path, mode="r|gz") as tf:
            ...
            for raw in f:
                ...
                if not first_snapshot_found:
                    if action == "snapshot":
                        ...
                        diag["first_snapshot_index"] = record_index
                        diag["first_snapshot_ts"] = ts
                    else:
                        diag["leading_updates_skipped"] += 1
                        continue
        ...
        if not first_snapshot_found:
            fail(f"{inst} no snapshot found in archive")


def normalize_doge(path: Path, out_csv: Path) -> dict:
    asks: dict = {}
    bids: dict = {}
    ap: list = []
    bp: list = []
    ...
    diag = {
        "leading_updates_skipped": 0,
        "first_snapshot_index": None,
    ...
    with tmp.open("w", encoding="utf-8", newline="") as out:
        ...
        def emit(boundary_ms: int) -> None:
            ...
            if not (
                ...
                fail("DOGE invalid normalized top-of-book")
        ...
        for t, group in itertools.groupby(
            iter_l2_repaired(path, DOGE, diag),
            key=lambda x: x[0],
        ):
            t = int(t)
            ...
            for _ts, action, aa, bb in rows:
                ...
                if not valid_book(asks, bids, ap, bp):
                    fail(f"DOGE invalid reconstructed book at {t}")
    ...
    return {
        "leading_updates_skipped": diag["leading_updates_skipped"],
        "first_snapshot_index": diag["first_snapshot_index"],
        "first_snapshot_ts": diag["first_snapshot_ts"],
        "records_applied_after_first_snapshot": records,
        "snapshot_records_applied": snapshots,
        "update_records_applied": updates,
...
def main() -> int:
    try:
        ...
        require_registry()
        d0 = require_d0()
        v01 = require_v01_parent()
        OUT_DIR.mkdir(parents=True, exist_ok=True)

        print("=== C7-D1 v0.2 VERIFY SIX INHERITED PASS ASSETS ===", flush=True)
        assets = verify_inherited_assets(v01)

        d0_assets = d0.get("assets") or {}
        doge_d0 = d0_assets.get(DOGE) or {}
        l2 = doge_d0.get("historical_l2") or {}
        meta = doge_d0.get("instrument") or {}

        filename = l2.get("filename")
        expected_size = int(l2.get("content_length", 0))
        uly = meta.get("uly")

        if filename != f"{DOGE}-L2orderbook-400lv-{FIXED_DATE}.tar.gz":
            fail("DOGE D0 filename mismatch")
        if expected_size <= 0:
            fail("DOGE D0 size missing")
        if not isinstance(uly, str) or not uly:
            fail("DOGE D0 uly missing")

        print("=== C7-D1 v0.2 DOGE SOURCE RE-VERIFY ===", flush=True)
        url = resolve_l2(DOGE, uly, filename)
        size_now = head_size(url, filename)
        if size_now != expected_size:
            fail(f"DOGE HEAD size changed: {size_now} != {expected_size}")

        path, reused = download_exact(url, filename, expected_size)
        digest = sha256_file(path)

        print("=== C7-D1 v0.2 DOGE REPLAY FROM FIRST SNAPSHOT ===", flush=True)
        norm_path = NORMALIZED_DIR / "DOGE_top1_1s.csv"
        norm = normalize_doge(path, norm_path)

        doge_pass = (
            norm["leading_updates_skipped"] == 3609
            and norm["first_snapshot_index"] == 3610
            and norm["first_snapshot_ts"] == 1707696060009
            and norm["valid_sampled_seconds"] >= 80_000
            and norm["active_utc_hours"] == 24
        )

        assets[DOGE] = {
            "integrity_pass": doge_pass,
            "inherited_from_v01": False,
            "repair_rule": "discard_leading_updates_until_first_snapshot",
            "source_archive": {
                "filename": filename,
        ...
        status = PASS if doge_pass and len(assets) == len(INSTS) else REVIEW
        ...
        report = {
            ...
            "universe": list(INSTS),
            "repair_rule": "discard_leading_updates_until_first_snapshot",
            "inherited_pass_assets": list(INHERITED),
            "repaired_asset": DOGE,
            "assets": assets,
            "assets_passed": sum(
        ...
        print("inherited_pass_assets =", len(INHERITED))
        print("DOGE leading_updates_skipped =", norm["leading_updates_skipped"])
        print("DOGE first_snapshot_index =", norm["first_snapshot_index"])
        print("DOGE first_snapshot_ts =", norm["first_snapshot_ts"])
        print("DOGE valid_sampled_seconds =", norm["valid_sampled_seconds"])
        print("DOGE active_utc_hours =", norm["active_utc_hours"])
