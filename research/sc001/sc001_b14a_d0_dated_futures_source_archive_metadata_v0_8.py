...
STAGE="SC001-B14A-D0-DATED-FUTURES-SOURCE-ARCHIVE-METADATA-V0.8"
PASS="B14A_D0_V08_SOURCE_ARCHIVE_METADATA_PASS"
REVIEW="B14A_D0_V08_SOURCE_ARCHIVE_METADATA_REVIEW"
SELFTEST_PASS="B14A_D0_V08_SELF_TEST_PASS"
SELFTEST_REVIEW="B14A_D0_V08_SELF_TEST_REVIEW"
...
PROTOCOL=ROOT/"docs/research/sc001-b14a-d0-dated-futures-source-archive-metadata-protocol-v0.6.md"
...
SEMANTICS_REVIEW=ROOT/"docs/research/sc001-b14a-futures-chain-archive-semantics-review-v0.1.md"
HANDSHAKE_REVIEW=ROOT/"docs/research/sc001-b14a-d0-v0.7-parent-identity-handshake-review-v0.1.md"
PROBE_LAG_DAYS=3
FREEZE=ROOT/"docs/research/sc001-b14a-d0-implementation-freeze-v0.8.json"

DATA_ROOT=Path(os.environ.get("SC001_DATA_ROOT",str(Path.home()/"sc001_data"))).expanduser().resolve()
OUT_DIR=DATA_ROOT/"SC001_B14A_D0_SOURCE_ARCHIVE"
OUT=OUT_DIR/"sc001_b14a_d0_source_archive_metadata_v0_8.json"
...
def require_freeze()->dict:
    fr=load_json(FREEZE)
    if fr.get("status")!="FROZEN_BEFORE_B14A_D0_V08_RUN":
    ...
    checks={
        ...
        "semantics_review_git_blob_sha":git_blob(SEMANTICS_REVIEW),
        "handshake_review_git_blob_sha":git_blob(HANDSHAKE_REVIEW),
    }
    ...
    for attempt in range(1,RETRIES+1):
        try:
            req=urllib.request.Request(url,headers={
                "User-Agent":"BotMarketplace-SC001-B14A-D0/0.8",
...
def resolve_family_archive(family:str,date_text:str)->tuple[str,str,dict]:
    basename=f"{family}-futureschain-trades-{date_text}.zip"
    ...
    for query_label,qd in query_days:
        ...
        for domain in OKX_DOMAINS:
            try:
                ...
                if not isinstance(obj,dict) or str(obj.get("code"))!="0":
                    fail(f"archive metadata code mismatch {family}: {obj.get('code') if isinstance(obj,dict) else None}")
    ...
    if len(unique)>1:
        fail(f"multiple exact futureschain archive URLs {family} {date_text}: {len(unique)}")
    raise RuntimeError(f"exact futureschain archive unresolved {basename}; attempts={attempts}")
...
def probe_family_archive(family:str)->dict:
    probe_date=probe_utc_date()
    try:
        fn,url,meta=resolve_family_archive(family,probe_date)
...
def self_test()->int:
    try:
        ...
        good="BTC-USD-futureschain-trades-2026-09-16.zip"
...
def run()->int:
    try:
        ...
        probe_date=probe_utc_date()
        family_archive_probes={
            family:probe_family_archive(family)
            for family in FAMILIES
        }
        archive_pass_counts={
            family:(1 if family_archive_probes[family]["archive_pass"] else 0)
            for family in FAMILIES
        }
        ...
        rep={
            ...
            "family_archive_probes":family_archive_probes,
        ...
        print("family_archive_files =",{k:v.get("archive_file") for k,v in family_archive_probes.items()})
