...
from sc001_b15p1_nonprice_transferability_lib_v0_1_4 import (
...
VERSION = "0.1.4"
SELFTEST_PASS = "B15P1_NONPRICE_COLLECTOR_V014_SELF_TEST_PASS"
SELFTEST_REVIEW = "B15P1_NONPRICE_COLLECTOR_V014_SELF_TEST_REVIEW"
...
IMPLEMENTATION_CONTRACT = ROOT / "docs/research/sc001-b15-p1-nonprice-collector-implementation-contract-v0.1.4.json"
...
SERVICE_FILE = ROOT / "ops/systemd/sc001-b15p1-transferability-v0.1.4.service"
FREEZE = ROOT / "docs/research/sc001-b15-p1-nonprice-collector-implementation-freeze-v0.1.4.json"
...
def require_freeze() -> dict[str, Any]:
    ...
    if fr.get("status") != "FROZEN_BEFORE_B15P1_NONPRICE_COLLECTOR_SELF_TEST_V014":
        fail("implementation freeze status mismatch")
    expected = {
        "runner_sha256": sha256_file(Path(__file__).resolve()),
        "library_sha256": sha256_file(
            Path(__file__).resolve().with_name(
                "sc001_b15p1_nonprice_transferability_lib_v0_1_4.py"
            )
        ),
...
def write_selftest_report(
    ...
    *,
    ...
    obj = {
        "schema": "sc001.b15.p1_nonprice_collector_implementation_self_test.v0.1.4",
        "status": status,
...
def self_test() -> int:
    ...
    try:
        ...
        # Bybit documents withdrawMax=-1 as "no limit". This exception is
        # deliberately field-specific and must not weaken generic decimal checks.
        by_unlimited = fixture_bybit_body()
        by_unlimited["result"]["rows"][0]["chains"][0]["withdrawMax"] = "-1"
        parsed_unlimited = parse_bybit_coin_info(by_unlimited)
        if parsed_unlimited[0].get("max_withdrawal") != "UNLIMITED":
            fail("Bybit withdrawMax=-1 unlimited sentinel fixture")

        by_bad_max = fixture_bybit_body()
        by_bad_max["result"]["rows"][0]["chains"][0]["withdrawMax"] = "-2"
        try:
            parse_bybit_coin_info(by_bad_max)
        except ValueError:
            pass
        else:
            fail("Bybit withdrawMax unsupported negative accepted")

        by_bad_fee = fixture_bybit_body()
        by_bad_fee["result"]["rows"][0]["chains"][0]["withdrawFee"] = "-1"
        try:
            parse_bybit_coin_info(by_bad_fee)
        except ValueError:
            pass
        else:
            fail("Bybit withdrawFee negative accepted")
        ...
        finite_after = dict(unlimited_before)
        finite_after["max_withdrawal"] = "100000"
        if "MAX_WITHDRAW_CHANGED" not in source_state_events(
            unlimited_before, finite_after
        ):
            fail("Bybit unlimited->finite max-withdraw transition fixture")
        ...
        from sc001_b15p1_nonprice_transferability_lib_v0_1_4 import validate_capability_snapshot
        validate_capability_snapshot(
            cap,
            runner_sha256=runner_sha,
