...
DIAG_LATENCY_MS = 1_000
DIAG_HORIZONS_MS = (30_000, 120_000)
...
ALL_TARGET_DAYS = DISCOVERY_DAYS + CONFIRM_DAYS
...
def nearest_rank(sorted_xs: list[float], q: float) -> float:
    if len(sorted_xs) != ROLLING_N:
        fail("nearest-rank window length mismatch")
    idx = math.ceil(q * ROLLING_N) - 1
...
def source_manifest() -> dict[str, dict]:
    ...
    for row in rows:
        ...
        if fn != expected_fn or not isinstance(size, int) or size <= 0 or not isinstance(digest, str) or len(digest) != 64:
            fail(f"bad source identity for {d}")
        path = ARCHIVES / fn
        out[d] = {"path": path, "filename": fn, "bytes": size, "sha256": digest}
    ...
    with zipfile.ZipFile(path, "r") as zf:
        ...
        with zf.open(members[0], "r") as raw:
            ...
            if header != EXPECTED_HEADER:
                fail(f"trade header mismatch in {path.name}: {header!r}")
            ...
            for row in reader:
                ...
                if side not in {"buy", "sell"} or not math.isfinite(px) or not math.isfinite(sz) or px <= 0 or sz <= 0:
                    fail(f"invalid target trade values in {path.name}: {row!r}")
                ...
                if source_last_id is not None and tid <= source_last_id:
                    fail(f"source target-instrument trade-id duplicate/backward in {path.name}")
                ...
                if day_start <= ts < day_end:
                    ...
                    k = (ts - day_start) // GRID_MS
                    n = px * sz
                    signed[k] += n if side == "buy" else -n
                    total[k] += n
                    timestamps.append(ts); prices.append(px)
...
def build_day_stream(date_text: str, manifest: dict[str, dict]) -> dict:
    ...
    last_state = {"ts": None, "id": None}
    read_archive_for_day(Path(manifest[date_text]["path"]), day_start, day_end, signed, total, timestamps, prices, stats, last_state)
    nd = next_day(date_text)
    read_archive_for_day(Path(manifest[nd]["path"]), day_start, day_end, signed, total, timestamps, prices, stats, last_state)
    if stats["admitted_rows"] <= 0:
        fail(f"no admitted rows for {date_text}")
    minute_seen = set((int(ts) - day_start) // 60_000 for ts in timestamps)
    if len(minute_seen) != 1440:
        fail(f"UTC minute coverage failure {date_text}: {len(minute_seen)}/1440")
...
def build_candidates(stream: dict) -> tuple[dict[str, list[dict]], dict]:
    ...
    out = {name: [] for name, _ in QS}
    valid_buckets = scores_built = 0
    threshold_ready_scores = 0
    tie_counts = {name: 0 for name, _ in QS}
    day_start = int(stream["day_start"]); day_end = int(stream["day_end"])
    signed = stream["signed"]; total = stream["total"]
    ...
    for k in range(len(total)):
        ...
        if len(activity_q) == ROLLING_N:
            ...
            if not math.isfinite(scale) or scale <= 0:
                fail("non-positive/non-finite activity scale")
            flow = float(signed[k]) / scale
            if not math.isfinite(flow):
                fail("non-finite FLOW_IMPULSE")
            scores_built += 1
            decision_ts = day_start + (k + 1) * GRID_MS
            if len(score_q) == ROLLING_N:
                threshold_ready_scores += 1
                af = abs(flow)
                for name, q in QS:
    ...
    diag = {"valid_buckets": valid_buckets, "flow_scores_built": scores_built,
            "threshold_ready_scores": threshold_ready_scores,
            "candidate_counts": {k: len(v) for k, v in out.items()},
...
def simulate_scenario(stream: dict, candidates: list[dict], latency_ms: int, horizon_ms: int) -> dict:
    timestamps = stream["timestamps"]; prices = stream["prices"]
    day_end = int(stream["day_end"])
    rows = []
    skipped = 0; ineligible_day_end = 0; unfilled = 0; open_until = -1
    for c in candidates:
        decision = int(c["decision_ts"])
        ...
        if xi is None:
            ...
            open_until = day_end
            continue
        ets = int(timestamps[ei]); xts = int(timestamps[xi])
        ep = float(prices[ei]); xp = float(prices[xi])
        direction = int(c["direction"])
        edge = direction * (xp / ep - 1.0) * 10_000.0
        rows.append({"decision_ts": decision, "direction": direction, "flow_impulse": float(c["flow_impulse"]),
                     "causal_threshold": float(c["threshold"]), "entry_target_ts": entry_target,
                     "entry_ts": ets, "entry_wait_ms": ets - entry_target, "entry_price": ep,
                     "exit_target_ts": exit_target, "exit_ts": xts, "exit_wait_ms": xts - exit_target,
                     "exit_price": xp, "gross_edge_bps": edge})
        open_until = xts
    eligible = len(candidates) - skipped - ineligible_day_end
    completed = len(rows)
    completion = completed / eligible if eligible > 0 else 0.0
    vals = [r["gross_edge_bps"] for r in rows]
    return {"raw_candidate_count": len(candidates), "skipped_while_open": skipped,
            "ineligible_day_end": ineligible_day_end, "eligible_nonoverlap_count": eligible,
            "unfilled_count": unfilled, "completed_trade_count": completed, "completion_rate": completion,
            ...
            "positive_trade_fraction": (sum(1 for v in vals if v > 0) / len(vals)) if vals else None,
            "entry_wait_p95_ms": percentile([r["entry_wait_ms"] for r in rows], 0.95),
            "exit_wait_p95_ms": percentile([r["exit_wait_ms"] for r in rows], 0.95),
            "rows": rows}
...
def worker(date_text: str, manifest: dict[str, dict], out_root: str, impl_sha: str) -> dict:
    root = Path(out_root); day_dir = root / "days" / date_text; day_dir.mkdir(parents=True, exist_ok=True)
    report_path = day_dir / "day_report.json"
    if report_path.exists():
        old = load_json(report_path)
        if old.get("status") == "PASS" and old.get("implementation_sha256") == impl_sha:
            print(f"[{date_text}] COMPLETE CHECKPOINT REUSED", flush=True)
            return old
    stream = build_day_stream(date_text, manifest)
    candidates, feature_diag = build_candidates(stream)
    scenarios = []
    for qname, lat, horizon, role in SCENARIOS:
        sm = simulate_scenario(stream, candidates[qname], lat, horizon)
        sm.pop("rows", None)
        sm.update({"threshold_scenario": qname, "latency_ms": lat, "horizon_ms": horizon, "role": role})
        scenarios.append(sm)
    rep = {"stage": STAGE, "version": VERSION, "status": "PASS", "date": date_text,
           "implementation_sha256": impl_sha, "source_stats": stream["stats"],
           "feature_diagnostics": feature_diag, "scenario_summaries": scenarios,
           "q2_okx_accessed": False, "validation_or_final_accessed": False,
           "l2_accessed": False, "do_not_interpret_before_stage_complete": True}
...
def aggregate(day_reports: list[dict]) -> dict[str, dict]:
    ...
    for rep in day_reports:
        ...
        for s in rep["scenario_summaries"]:
            ...
            by_sid.setdefault(sid, []).append((d, s))
    out = {}
    for qname, lat, horizon, role in SCENARIOS:
        sid = scenario_key(qname, lat, horizon)
        rows = sorted(by_sid.get(sid, []))
        if len(rows) != len(day_reports):
            fail(f"aggregate missing days for {sid}")
        completed = sum(int(x[1]["completed_trade_count"]) for x in rows)
        eligible = sum(int(x[1]["eligible_nonoverlap_count"]) for x in rows)
        daily_means = [x[1]["mean_gross_edge_bps"] for x in rows]
        # Recreate pooled values from day means weighted by completed count for mean.
        weighted_num = sum((float(x[1]["mean_gross_edge_bps"]) if x[1]["mean_gross_edge_bps"] is not None else 0.0) * int(x[1]["completed_trade_count"]) for x in rows)
        pooled_mean = weighted_num / completed if completed else None
        # Pooled median requires exact trade rows; compute it in a second deterministic pass from per-day stream.
        out[sid] = {"threshold_scenario": qname, "latency_ms": lat, "horizon_ms": horizon, "role": role,
                    ...
                    "median_daily_mean_gross_edge_bps": statistics.median(daily_means) if all(v is not None for v in daily_means) else None,
                    "positive_daily_mean_days": sum(1 for v in daily_means if v is not None and v > 0),
                    "all_days_have_completed_trades": all(v is not None for v in daily_means)}
    return out


def pooled_trade_edges(days: tuple[str, ...], manifest: dict[str, dict], qname: str, lat: int, horizon: int) -> list[float]:
    # Used only once per primary aggregate to obtain the exact pooled median.
    edges = []
    for d in days:
        stream = build_day_stream(d, manifest)
        candidates, _ = build_candidates(stream)
        sm = simulate_scenario(stream, candidates[qname], lat, horizon)
        edges.extend(float(r["gross_edge_bps"]) for r in sm["rows"])
    return edges


def gates_for(stage: str, agg: dict[str, dict], pooled_primary_edges: list[float]) -> tuple[str, dict]:
    p = agg[scenario_key("q99.5", 250, 60_000)]
    s = agg[scenario_key("q99.5", 500, 60_000)]
    min_days = 14 if stage == "discovery" else 7
    min_trades = 200 if stage == "discovery" else 90
    checks = {
        "pooled_mean_ge_12bps": p["pooled_mean_gross_edge_bps"] is not None and p["pooled_mean_gross_edge_bps"] >= 12.0,
        ...
        "pooled_median_positive": bool(pooled_primary_edges) and statistics.median(pooled_primary_edges) > 0,
    ...
    if stage == "discovery":
        verdict = "E003_DISCOVERY_PASS" if all(checks.values()) else "E003_DISCOVERY_FAIL"
    else:
        verdict = "E003_GROSS_HURDLE_PASS" if all(checks.values()) else "E003_CONFIRMATION_FAIL"
    p["pooled_median_gross_edge_bps"] = statistics.median(pooled_primary_edges) if pooled_primary_edges else None
...
def write_summary(path: Path, stage: str, verdict: str, agg: dict[str, dict], checks: dict) -> None:
    p = agg[scenario_key("q99.5", 250, 60_000)]
    s = agg[scenario_key("q99.5", 500, 60_000)]
    lines = [f"# SC001-E003 {stage.title()} Flow-Impulse Screen", "", f"- Verdict: `{verdict}`",
             "- Q2 / Validation / Final: **NO**", "- L2 accessed: **NO**", "",
             "## Primary q99.5 / 250 ms / 60 s",
             f"- completed trades: {p['completed_trade_count']}",
             f"- completion rate: {p['completion_rate']}",
             f"- pooled mean gross edge bps: {p['pooled_mean_gross_edge_bps']}",
             f"- pooled median gross edge bps: {p.get('pooled_median_gross_edge_bps')}",
             f"- median daily mean gross edge bps: {p['median_daily_mean_gross_edge_bps']}",
             f"- positive daily mean days: {p['positive_daily_mean_days']}", "",
             "## 500 ms stress", f"- pooled mean gross edge bps: {s['pooled_mean_gross_edge_bps']}", "",
             "## Gates"]
    for k, v in checks.items():
        lines.append(f"- {k}: {v}")
    lines += ["", "## Boundary",
              "This is a coarse trade-price gross-economics screen only. It does not prove executable profitability and does not include L2 spread/depth or fee charging."]
    atomic_text(path, "\n".join(lines) + "\n")
...
def preflight() -> dict[str, dict]:
    ...
    print("discovery_days =", list(DISCOVERY_DAYS))
    print("confirmation_days =", list(CONFIRM_DAYS))
    print("primary = q99.5 / 250ms / 60s")
    print("stress = q99.5 / 500ms / 60s")
    print("rolling activity = 720 valid buckets; rolling threshold = 720 prior valid scores")
    print("candidate inequality = strict >")
...
def run_stage(stage: str, manifest: dict[str, dict]) -> None:
    if stage == "discovery":
        ...
        if final_path.exists():
            fail("discovery terminal report already exists; refusing overwrite")
    else:
        ...
        if final_path.exists():
            fail("confirmation terminal report already exists; refusing overwrite")
    ...
    with ProcessPoolExecutor(max_workers=4) as pool:
        ...
        for fut in as_completed(futs):
            d = futs[fut]
            rep = fut.result(); reports.append(rep)
            print(f"[{d}] worker complete (withheld from interpretation until stage complete)", flush=True)
    if len(reports) != len(days) or any(r.get("status") != "PASS" for r in reports):
        fail("not all day workers completed PASS")
    reports.sort(key=lambda r: r["date"])
    agg = aggregate(reports)
    # Exact pooled median for the primary gate. This rereads only the already-open stage days.
    primary_edges = pooled_trade_edges(days, manifest, "q99.5", 250, 60_000)
    verdict, checks = gates_for(stage, agg, primary_edges)
    report = {"stage": STAGE, "version": VERSION, "research_stage": stage,
              "protocol": PROTOCOL, "implementation_freeze": IMPLEMENTATION_FREEZE,
              "implementation_sha256": impl_sha, "verdict": verdict,
              "days": list(days), "day_reports": [str(root / "days" / d / "day_report.json") for d in days],
              "aggregates": agg, "gate_checks": checks,
              "q2_okx_accessed": False, "validation_or_final_accessed": False, "l2_accessed": False,
              "e002_tfi_used": False, "finished_at_utc": datetime.now(timezone.utc).isoformat()}
    atomic_json(final_path, report)
    write_summary(summary_path, stage, verdict, agg, checks)
    print(f"COMPLETE: {verdict}")


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("mode", choices=("preflight", "discovery", "confirmation"))
    args = ap.parse_args()
    manifest = preflight()
    if args.mode == "preflight":
        return
    run_stage(args.mode, manifest)
