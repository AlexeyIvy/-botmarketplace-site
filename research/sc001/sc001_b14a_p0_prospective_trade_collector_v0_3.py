...
SELFTEST_PASS="B14A_P0_COLLECTOR_V03_SELF_TEST_PASS"
SELFTEST_REVIEW="B14A_P0_COLLECTOR_V03_SELF_TEST_REVIEW"
...
LEGACY_STAGE="SC001-B14A-P0-PROSPECTIVE-TRADE-COLLECTOR-V0.1"

ROOT=Path(__file__).resolve().parents[2]
PROTOCOL=ROOT/"docs/research/sc001-b14a-p0-collector-operational-resilience-v0.2.md"
REGISTRY=ROOT/"docs/research/sc001-contamination-registry-v0.29.json"
FREEZE=ROOT/"docs/research/sc001-b14a-p0-collector-implementation-freeze-v0.3.json"
...
def require_freeze():
    fr=load_json(FREEZE)
    if fr.get("status")!="FROZEN_BEFORE_B14A_P0_COLLECTION_V03":
...
def request_json(url):
    req=urllib.request.Request(url,headers={
        "User-Agent":"BotMarketplace-SC001-B14A-P0/0.3",
...
def load_existing_state()->tuple[dict|None,bool]:
    if not STATE.exists():
        return None,False
    s=load_json(STATE)
    if s.get("stage") not in {STAGE,LEGACY_STAGE}:
        fail("existing P0 state stage mismatch")
    migrated=s.get("stage")==LEGACY_STAGE
    s["stage"]=STAGE
    s.setdefault("process_restart_count",0)
    s.setdefault("process_restart_gaps",[])
    s.setdefault("last_heartbeat_ms",None)
    s.setdefault("raw_trade_messages",0)
    s.setdefault("normalized_trade_rows",0)
    s.setdefault("invalid_trade_rows",0)
    s.setdefault("reconnect_count",0)
    s.setdefault("connection_gaps",[])
    s.setdefault("subscription_ack",False)
    return s,migrated
...
def record_process_restart_gap(state:dict,migrated_from_v01:bool)->None:
    t=now_ms()
    prev=state.get("last_heartbeat_ms")
    if migrated_from_v01:
        # v0.1 had no reliable process heartbeat; the one-time migration
        # is recorded externally by the systemd migration script.
        state["legacy_v01_migration_seen"]=True
    elif prev is not None:
        prev=int(prev)
        gap=max(0,t-prev)
        rec={"start_ms":prev,"end_ms":t,"duration_ms":gap}
        state["process_restart_count"]=int(state.get("process_restart_count",0))+1
        state.setdefault("process_restart_gaps",[]).append(rec)
        log_conn("PROCESS_RESTART_GAP","process",**rec)
...
def self_test():
    try:
        ...
        print("resilience = persistent heartbeat+restart gap ledger")
...
def wait_until(target_ms,state):
    while not STOP:
        rem=target_ms-now_ms()
        if rem<=0:
            return
        touch_heartbeat(state)
        time.sleep(min(HEARTBEAT_SECONDS,max(1,rem/1000)))
...
def collect():
    ...
    current=now_ms()
    OUT_DIR.mkdir(parents=True,exist_ok=True)
    prior,migrated_from_v01=load_existing_state()

    if prior is not None and prior.get("status")==COMPLETE:
        print(COMPLETE)
        print("existing_complete_state = True")
    ...
    if current>CAPTURE_END_MS:
        if prior is not None:
            ...
            prior["version"]="0.3"
            prior["status"]=REVIEW
            prior["finished_ms"]=current
            prior["finished_utc"]=iso(current)
            prior["last_heartbeat_ms"]=current
            save_state(prior)
        print(REVIEW)
        print("error = capture window ended without COMPLETE; terminal no-retry state")
        print("basis/convergence/PnL = False")
        return 0
    ...
    if prior is None:
        ...
        state=prior
        state["version"]="0.3"
        state["instrument_metadata"]=metadata
        state["status"]=WAITING if current<CONNECT_AT_MS else RUNNING
        record_process_restart_gap(state,migrated_from_v01)
    ...
    if current<CONNECT_AT_MS:
        ...
        wait_until(CONNECT_AT_MS,state)
    ...
    while not STOP and now_ms()<=CAPTURE_END_MS+5000:
        ...
        try:
            ...
            ws=websocket.create_connection(
                ...
                header=["User-Agent: BotMarketplace-SC001-B14A-P0/0.3"],
            )
            ws.settimeout(1.0)
            ...
            state["last_disconnect_ms"]=last_disconnect
            state["last_heartbeat_ms"]=last_disconnect
            save_state(state)
    ...
    state["last_heartbeat_ms"]=finished
