...
STAGE = "SC001-B13A-FUNDING-DIFFERENTIAL-STRUCTURAL-PREFLIGHT-V0.3"
...
PROTOCOL = ROOT / "docs/research/sc001-b13a-funding-differential-structural-preflight-v0.2.md"
...
FREEZE = ROOT / "docs/research/sc001-b13a-funding-differential-implementation-freeze-v0.3.json"

DATA_ROOT = Path(os.environ.get("SC001_DATA_ROOT", str(Path.home()/"sc001_data"))).expanduser().resolve()
OUT_DIR = DATA_ROOT / "SC001_B13A_FUNDING_PREFLIGHT"
OKX_ARCH = OUT_DIR / "okx_archives"
OUT = OUT_DIR / "sc001_b13a_funding_differential_structural_preflight_v0_3.json"
...
def require_freeze() -> None:
    fr = load_json(FREEZE)
    if fr.get("status") != "FROZEN_BEFORE_B13A_STRUCTURAL_PREFLIGHT_V03_RUN":
    ...
    for attempt in range(1, RETRIES+1):
        try:
            req = urllib.request.Request(url, headers={
                "User-Agent":"BotMarketplace-SC001-B13A/0.3",
...
def okx_archive_metadata(symbol: str) -> list[dict]:
    ...
    for node in walk_file_nodes(obj.get("data")):
        ...
        files[filename] = {
            "filename":filename,
            "url":url,
            "sizeMB_raw":node.get("sizeMB"),
        }

    if not files:
        fail(f"no trusted OKX funding archive metadata returned for {symbol}")
