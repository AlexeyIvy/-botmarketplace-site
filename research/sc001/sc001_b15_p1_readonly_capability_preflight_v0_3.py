...
STAGE="SC001-B15-P1-READONLY-CAPABILITY-PREFLIGHT-V0.3"
PASS="B15_P1_READONLY_CAPABILITY_PASS"
REVIEW="B15_P1_READONLY_CAPABILITY_REVIEW"

ROOT=Path(__file__).resolve().parents[2]
PROTOCOL=ROOT/"docs/research/sc001-b15-p1-readonly-capability-diagnostic-v0.3.md"
SECURITY=ROOT/"docs/research/sc001-b15-p1-read-only-credential-security-contract-v0.1.md"
FREEZE=ROOT/"docs/research/sc001-b15-p1-readonly-capability-implementation-freeze-v0.3.json"
...
def require_freeze()->dict:
    ...
    if fr.get("status")!="FROZEN_BEFORE_B15_P1_READONLY_CAPABILITY_DIAGNOSTIC_V03":
...
# B15-only transport policy:
# keep authenticated REST egress deterministic and compatible with
# exchange IPv4 API-key whitelists without changing system-wide routing.
socket.getaddrinfo=ipv4_only_getaddrinfo

def http_json(url:str,headers:dict[str,str]|None=None)->dict:
    req=urllib.request.Request(url,headers=headers or {
        "User-Agent":"BotMarketplace-SC001-B15-P1/0.3",
...
def safe_body_summary(body:str)->str:
    try:
        obj=json.loads(body)
        if isinstance(obj,dict):
            safe={}
            for k in ("code","msg","retCode","retMsg","retExtInfo"):
                if k in obj:
                    safe[k]=obj.get(k)
            if safe:
...
def run_one(label,fn):
    try:
        ...
        print(label,"= REVIEW")
        print(label,"http_status =",exc.status)
        print(label,"safe_body =",safe_body_summary(exc.body))
        return False,None
    except Exception as exc:
        print(label,"= REVIEW")
        print(label,"error =",f"{type(exc).__name__}: {exc}")
        return False,None
...
def main()->int:
    ...
    try:
        ...
        print(REVIEW)
        print("setup error =",f"{type(exc).__name__}: {exc}")
        print("transport_family = IPv4-only")
        print("secret_values_printed = False")
        return 2

    okx_ok,okx=run_one("OKX",lambda:check_okx(cfg))
    bybit_ok,bybit=run_one("BYBIT",lambda:check_bybit(cfg))
    ...
    if all_ok:
        ...
        if not clock_ok:
            print("CLOCK = REVIEW")
            print("clock skew exceeds",MAX_CLOCK_SKEW_MS,"ms")

    print(PASS if all_ok else REVIEW)
    print("secret_values_printed = False")
