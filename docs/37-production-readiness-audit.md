...
### 5.12. [LOW] ~~Нет "DSL dry-run" preview перед запуском~~ [CLOSED]
...
**Статус.** Закрыто тремя PR:
- **#286** — `POST /api/v1/lab/preview`: синхронный stateless dry-run через существующий pure-function `runBacktest()` против последних N часов (≤168) `MarketCandle`. Принимает `dslJson` либо `strategyVersionId` (workspace-scoped). Rate limit 5/min per IP. Body: 400 на DSL/symbol/hours, 404 на cross-tenant version, 409 на insufficient candles, 422 на engine throw.
- **#287** — UI: кнопка "Preview 24h" в Build context bar рядом с Compile & Save; popover с trades / winrate / max drawdown / net PnL + inline SVG equity sparkline.
- **#288** — та же кнопка в Test tab для A/B-сравнения версий перед полным backtest; попап открывается влево чтобы не клипать форму.

Контракт задокументирован в `docs/openapi/openapi.yaml` (POST /lab/preview, схемы `PreviewRequest` / `PreviewResponse`).
