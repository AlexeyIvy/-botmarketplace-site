...
- **Equity Curve** — линейный график изменения эквити (нормализованный, стартовая точка 100) на основе `tradeLog.pnlPct`, отображаемый через `lightweight-charts` во вкладке "Equity Curve" результатов бэктеста. Введён в Phase B1 (B1-3).

- **Onboarding State** — состояние пустого канваса (`nodes.length === 0`) в Build-табе. Показывает подсказку и кнопку "Load EMA Crossover example" для загрузки шаблонного графа. Введено в Phase B1 (B1-1).

- **Graph Rename** — функция переименования графа инлайн двойным кликом по имени в selector bar. Сохраняет через `PATCH /api/v1/lab/graphs/:id` с `{ name }`. Введено в Phase B1 (B1-2).
