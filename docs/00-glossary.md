...
- **LabWorkspace** — личная рабочая область пользователя для построения стратегий в визуальном редакторе. **Не путать с `Workspace`** (см. ниже). Один на пользователя в Phase 3. Хранит ссылку на активный `StrategyGraph` и активный `MarketDataset`. Введена в Phase 3 Lab v2.

- **Workspace** — будущий мультиарендный контейнер (post-MVP). Используется в документах как `workspaceId` — планируемое расширение авторизации. **Не является** `LabWorkspace`. Пока не реализован.

  > **Правило разграничения:** `LabWorkspace` = инструментальная рабочая область для Lab v2. `Workspace` = будущий мультиарендный контейнер для всего приложения. Не использовать термины взаимозаменяемо.
