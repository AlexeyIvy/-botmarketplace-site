...
echo
echo "=== 4. VERIFY PROFILE HAS NO RUNTIME-FORBIDDEN SECTIONS ==="
if grep -Eq '^[[:space:]]*(cloudflared|admin_ui):' "$PROFILE"; then
  echo "FAIL: runtime profile still contains full-client/cloudflared-only section"
  sudo sed -n '1,220p' "$PROFILE" | sed '/api_key:/s#file:.*#file:[REDACTED]#'
  exit 125
else
  echo "PASS: runtime profile contains no cloudflared/admin_ui sections"
fi

echo
echo "=== 5. RUNTIME-ONLY PREFLIGHT (EXACT PRODUCTION BINARY) ==="
...
for _ in $(seq 1 60); do
  if [ -s "$PREFLIGHT_URL" ]; then
    PREFLIGHT_BASE="$(cat "$PREFLIGHT_URL")"
    break
  fi
...
if [ "$PREFLIGHT_READY" -ne 1 ]; then
  ...
  echo
  echo "--- /health?details=true ---"
  curl -sS --max-time 2 "$PREFLIGHT_BASE/health?details=true" || true
  echo
  echo "--- runtime log ---"
  cat "$PREFLIGHT_LOG" || true
...
echo
echo "=== 6. INSTALL READINESS + SYSTEMD UNIT ==="
...
echo
echo "=== 7. START SERVICE WITH READINESS GATE ==="
...
echo
echo "=== 12. VERIFY SECRET SEPARATION ==="
